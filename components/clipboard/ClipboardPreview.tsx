...
export function ClipboardPreview({ entry, bridge, onChanged, onError, onSave, onInteract, onExpandImage, onCreated, onDraft, recovered, onRecoveryCleared }: { entry: ClipboardDetail; bridge: ClipboardBridge; onChanged: () => void; onError: (error: unknown) => void; onSave?: (entry: ClipboardDetail) => Promise<void>; onInteract?: () => void; onExpandImage?: () => void; onCreated: (id: string) => Promise<void>; onDraft: (draft: ClipboardDraft | null) => void; recovered?: ClipboardRecoveryDraft | null; onRecoveryCleared: () => void }) {
  ...
  return <article className={styles.preview} aria-label={tr("剪贴板预览")} onFocus={onInteract}>
    ...
    {entry.kind === "image" ? <div className={styles.previewTools}>{onExpandImage ? <button onClick={onExpandImage}><AliIcon name="expand" size={14} />{tr("查看大图")}</button> : null}<button onClick={() => setZoom(0)}>{tr("适应窗口")}</button><button onClick={() => setZoom(1)}>100%</button><select aria-label={tr("图片缩放")} value={zoom} onChange={event => setZoom(Number(event.target.value))}><option value={0}>{tr("适应")}</option><option value={0.5}>50%</option><option value={1}>100%</option><option value={2}>200%</option></select></div> : null}
