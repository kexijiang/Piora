...
import { ClipboardImage, ClipboardPreview } from "./ClipboardPreview";
  ...
  return <section className={styles.workspace} data-surface={surface} data-locale={locale} data-preview={view.preview} data-detail={narrowDetail} data-multiple={selected.length > 0} aria-label={surface === "shelf" ? tr("屏幕暂存") : tr("剪贴板")} onKeyDown={keyDown}>
    ...
    <footer className={styles.actionBar}>
      ...
      <div className={styles.actions}>{view.filter === "trash" ? <><button className={styles.secondary} disabled={!ids.length || busy || storageUnavailable} onClick={() => void mutate({ type: "restore", ids }, tr("已恢复"))}>{tr("恢复")}</button><button className={styles.danger} disabled={!ids.length || busy || storageUnavailable} onClick={() => setConfirm({ type: "purge", ids })}>{tr("永久删除…")}</button></> : <>
        {surface !== "quick" || selected.length > 0 ? <button className={styles.secondary} disabled={!ids.length || busy || storageUnavailable || mixedSelection} onClick={() => void execute(false)}><AliIcon name="copy" size={16} />{selected.length > 1 ? tr("合并复制") : tr("复制")}</button> : null}
        ...
        <div className={styles.pasteAction}>{status?.target && surface !== "shelf" ? <span title={tr("粘贴到 {target}", { target: status.target })}>{surface === "manager" ? tr("粘贴到 ") : ""}{status.target}</span> : null}<button className={styles.primary} disabled={!ids.length || busy || storageUnavailable || mixedSelection} onClick={() => void execute(true)}>{selected.length > 1 ? tr("合并粘贴") : tr("粘贴")}<kbd>↵</kbd></button></div>
      </>}</div>
    </footer>
    ...
    {merge !== null && !draftGuard.open ? <div ref={modal} className={styles.overlay} role="dialog" aria-label={tr("合并预览")} aria-modal="true"><div className={styles.dialog}><h3>{tr("按选择顺序合并")}</h3><pre>{merge}</pre><div><button onClick={() => void execute(true)}>{tr("粘贴")}</button><button onClick={() => void execute(false)}>{tr("复制")}</button><button onClick={() => setMerge(null)}>{tr("关闭")}</button></div></div></div> : null}
  </section>;
}
