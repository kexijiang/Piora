...
export function SmartShellPanel({ cwd, sessionId, onClose, onToChat, onOpenFile, onOpenUrl, onSettings }: SmartShellPanelProps) {
  ...
  return <section className={styles.root} aria-label={t("shell.title")}>
    ...
    <div className={styles.locationBar}><span className={styles.cwd} title={state?.cwd || cwd}><AliIcon name="folder" size={14} />{state?.cwd || cwd}</span><span className={styles.status} data-ready={state?.connected && shell.connected}><i />{shell.connectionError ? t("shell.offline") : state?.profile.label || t("shell.connecting")}</span></div>
