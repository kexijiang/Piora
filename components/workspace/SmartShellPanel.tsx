...
export function SmartShellPanel({ cwd, sessionId, onClose, onToChat, onOpenFile, onOpenUrl, onSettings }: SmartShellPanelProps) {
  ...
  return <section className={styles.root} aria-label={t("shell.title")}>
    ...
    <div className={styles.body}>
      {state ? <div hidden={!native} className={styles.native}><TerminalSurface key={state.id} cwd={state.initialCwd} terminalId={state.id} inputEnabled={state.owner === "human"} onError={shell.setError} /></div> : null}
