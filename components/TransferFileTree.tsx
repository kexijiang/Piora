...
export function TransferFileTree({ items, selectedId, folderId, query, onQuery, onOpen, onFolder, onMove, onCreateFolder, onRenameFolder, onDeleteFolder, disabled }: {
  ...
  return <nav className={styles.fileSidebar} aria-label="文件夹和文件">
    ...
    <div className={styles.fileTree}>
      {rows.map(({ item, depth }) => <button key={item.id} type="button" className={styles.treeItem} style={{ paddingLeft: 10 + depth * 14 }} title={item.title}
