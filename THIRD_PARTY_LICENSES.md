# Third-party package license inventory

> Deterministically generated from the committed npm lockfile. Do not edit by hand; run `npm run licenses:generate`.

Lockfile SHA-256: `37f7f3d8220c1be8f75b85f458ce39336c203528d1c691017596c59de1114c46`

Unique locked packages: **1271**. Runtime packages: **825**. Build/development-only packages: **446**.

This source inventory records lockfile package-declared license labels plus exact version-scoped reviewed declarations before reviewed postinstall replacements. The packaged application additionally contains a build-derived SBOM, the final package-copy inventory, every published LICENSE/LICENCE/COPYING/NOTICE file, and version-scoped reviewed upstream fallbacks when a compiled npm package omits its required license text. A runtime package marked `UNDECLARED` fails generation.

Every locked package declares a license.

## Runtime dependency closure

| Package | Version | Declared license | Optional |
| --- | --- | --- | --- |
| `@ant-design/cssinjs` | `2.1.2` | MIT | No |
| `@antfu/install-pkg` | `1.1.0` | MIT | No |
| `@anthropic-ai/sdk` | `0.91.1` | MIT | No |
| `@aws-crypto/crc32` | `5.2.0` | Apache-2.0 | No |
| `@aws-crypto/sha256-browser` | `5.2.0` | Apache-2.0 | No |
| `@aws-crypto/sha256-js` | `5.2.0` | Apache-2.0 | No |
| `@aws-crypto/supports-web-crypto` | `5.2.0` | Apache-2.0 | No |
| `@aws-crypto/util` | `5.2.0` | Apache-2.0 | No |
| `@aws-sdk/client-bedrock-runtime` | `3.1048.0` | Apache-2.0 | No |
| `@aws-sdk/core` | `3.974.11` | Apache-2.0 | No |
| `@aws-sdk/core` | `3.977.9` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-env` | `3.972.37` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-env` | `3.972.70` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-http` | `3.972.39` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-http` | `3.972.72` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-ini` | `3.972.41` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-ini` | `3.973.15` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-login` | `3.972.41` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-login` | `3.972.77` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-node` | `3.972.42` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-node` | `3.972.81` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-process` | `3.972.37` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-process` | `3.972.70` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-sso` | `3.972.41` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-sso` | `3.973.14` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-web-identity` | `3.972.41` | Apache-2.0 | No |
| `@aws-sdk/credential-provider-web-identity` | `3.972.76` | Apache-2.0 | No |
| `@aws-sdk/eventstream-handler-node` | `3.972.16` | Apache-2.0 | No |
| `@aws-sdk/eventstream-handler-node` | `3.972.34` | Apache-2.0 | No |
| `@aws-sdk/middleware-eventstream` | `3.972.12` | Apache-2.0 | No |
| `@aws-sdk/middleware-eventstream` | `3.972.29` | Apache-2.0 | No |
| `@aws-sdk/middleware-websocket` | `3.972.19` | Apache-2.0 | No |
| `@aws-sdk/middleware-websocket` | `3.972.52` | Apache-2.0 | No |
| `@aws-sdk/nested-clients` | `3.997.44` | Apache-2.0 | No |
| `@aws-sdk/nested-clients` | `3.997.9` | Apache-2.0 | No |
| `@aws-sdk/signature-v4-multi-region` | `3.996.27` | Apache-2.0 | No |
| `@aws-sdk/signature-v4-multi-region` | `3.996.46` | Apache-2.0 | No |
| `@aws-sdk/token-providers` | `3.1048.0` | Apache-2.0 | No |
| `@aws-sdk/token-providers` | `3.1116.0` | Apache-2.0 | No |
| `@aws-sdk/types` | `3.973.8` | Apache-2.0 | No |
| `@aws-sdk/types` | `3.974.5` | Apache-2.0 | No |
| `@aws-sdk/util-locate-window` | `3.965.10` | Apache-2.0 | No |
| `@aws-sdk/util-locate-window` | `3.965.5` | Apache-2.0 | No |
| `@aws-sdk/xml-builder` | `3.972.24` | Apache-2.0 | No |
| `@aws-sdk/xml-builder` | `3.972.40` | Apache-2.0 | No |
| `@aws/lambda-invoke-store` | `0.2.4` | Apache-2.0 | No |
| `@aws/lambda-invoke-store` | `0.3.0` | Apache-2.0 | No |
| `@babel/code-frame` | `7.29.7` | MIT | No |
| `@babel/generator` | `7.29.8` | MIT | No |
| `@babel/helper-globals` | `7.29.7` | MIT | No |
| `@babel/helper-module-imports` | `7.29.7` | MIT | No |
| `@babel/helper-string-parser` | `7.29.7` | MIT | No |
| `@babel/helper-validator-identifier` | `7.29.7` | MIT | No |
| `@babel/parser` | `7.29.8` | MIT | No |
| `@babel/runtime` | `7.29.2` | MIT | No |
| `@babel/template` | `7.29.7` | MIT | No |
| `@babel/traverse` | `7.29.8` | MIT | No |
| `@babel/types` | `7.29.8` | MIT | No |
| `@braintree/sanitize-url` | `7.1.2` | MIT | No |
| `@chevrotain/types` | `11.1.2` | Apache-2.0 | No |
| `@codemirror/autocomplete` | `6.20.3` | MIT | No |
| `@codemirror/commands` | `6.11.0` | MIT | No |
| `@codemirror/lang-css` | `6.3.1` | MIT | No |
| `@codemirror/lang-html` | `6.4.12` | MIT | No |
| `@codemirror/lang-javascript` | `6.2.5` | MIT | No |
| `@codemirror/lang-json` | `6.0.2` | MIT | No |
| `@codemirror/lang-markdown` | `6.5.2` | MIT | No |
| `@codemirror/language` | `6.12.4` | MIT | No |
| `@codemirror/lint` | `6.9.7` | MIT | No |
| `@codemirror/search` | `6.7.2` | MIT | No |
| `@codemirror/state` | `6.7.2` | MIT | No |
| `@codemirror/view` | `6.43.10` | MIT | No |
| `@earendil-works/pi-agent-core` | `0.84.3` | MIT | No |
| `@earendil-works/pi-ai` | `0.84.3` | MIT | No |
| `@earendil-works/pi-client` | `0.84.3` | MIT | No |
| `@earendil-works/pi-coding-agent` | `0.84.3` | MIT | No |
| `@earendil-works/pi-protocol` | `0.84.3` | MIT | No |
| `@earendil-works/pi-telemetry` | `0.84.3` | MIT | No |
| `@earendil-works/pi-tui` | `0.84.3` | MIT | No |
| `@emnapi/runtime` | `1.11.3` | MIT | Yes |
| `@emotion/babel-plugin` | `11.13.5` | MIT | No |
| `@emotion/cache` | `11.14.0` | MIT | No |
| `@emotion/css` | `11.13.5` | MIT | No |
| `@emotion/hash` | `0.8.0` | MIT | No |
| `@emotion/hash` | `0.9.2` | MIT | No |
| `@emotion/memoize` | `0.9.0` | MIT | No |
| `@emotion/react` | `11.14.0` | MIT | No |
| `@emotion/serialize` | `1.3.3` | MIT | No |
| `@emotion/sheet` | `1.4.0` | MIT | No |
| `@emotion/unitless` | `0.10.0` | MIT | No |
| `@emotion/unitless` | `0.7.5` | MIT | No |
| `@emotion/use-insertion-effect-with-fallbacks` | `1.2.0` | MIT | No |
| `@emotion/utils` | `1.4.2` | MIT | No |
| `@emotion/weak-memoize` | `0.4.0` | MIT | No |
| `@google/genai` | `1.52.0` | Apache-2.0 | No |
| `@grpc/grpc-js` | `1.14.4` | Apache-2.0 | No |
| `@grpc/proto-loader` | `0.8.1` | Apache-2.0 | No |
| `@hono/node-server` | `2.1.1` | MIT | No |
| `@iconify/types` | `2.0.0` | MIT | No |
| `@iconify/utils` | `3.1.1` | MIT | No |
| `@img/colour` | `1.1.0` | MIT | No |
| `@img/sharp-darwin-arm64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-darwin-x64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-freebsd-wasm32` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-libvips-darwin-arm64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-darwin-x64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-arm` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-arm64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-ppc64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-riscv64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-s390x` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linux-x64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linuxmusl-arm64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-libvips-linuxmusl-x64` | `1.3.3` | LGPL-3.0-or-later | Yes |
| `@img/sharp-linux-arm` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linux-arm64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linux-ppc64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linux-riscv64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linux-s390x` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linux-x64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linuxmusl-arm64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-linuxmusl-x64` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-wasm32` | `0.35.4` | Apache-2.0 AND LGPL-3.0-or-later AND MIT | Yes |
| `@img/sharp-webcontainers-wasm32` | `0.35.4` | Apache-2.0 | Yes |
| `@img/sharp-win32-arm64` | `0.35.4` | Apache-2.0 AND LGPL-3.0-or-later | Yes |
| `@img/sharp-win32-ia32` | `0.35.4` | Apache-2.0 AND LGPL-3.0-or-later | Yes |
| `@img/sharp-win32-x64` | `0.35.4` | Apache-2.0 AND LGPL-3.0-or-later | Yes |
| `@isaacs/cliui` | `8.0.2` | ISC | No |
| `@isaacs/fs-minipass` | `4.0.1` | ISC | No |
| `@jridgewell/gen-mapping` | `0.3.13` | MIT | No |
| `@jridgewell/resolve-uri` | `3.1.2` | MIT | No |
| `@jridgewell/sourcemap-codec` | `1.5.5` | MIT | No |
| `@jridgewell/trace-mapping` | `0.3.31` | MIT | No |
| `@js-sdsl/ordered-map` | `4.4.2` | MIT | No |
| `@koromix/koffi-android-arm64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-android-x64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-darwin-arm64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-darwin-x64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-freebsd-arm64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-freebsd-ia32` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-freebsd-x64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-arm` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-arm64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-ia32` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-loong64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-riscv64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-linux-x64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-openbsd-ia32` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-openbsd-x64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-win32-arm64` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-win32-ia32` | `3.2.1` | MIT | Yes |
| `@koromix/koffi-win32-x64` | `3.2.1` | MIT | Yes |
| `@lezer/common` | `1.5.2` | MIT | No |
| `@lezer/css` | `1.3.6` | MIT | No |
| `@lezer/highlight` | `1.2.3` | MIT | No |
| `@lezer/html` | `1.3.13` | MIT | No |
| `@lezer/javascript` | `1.5.4` | MIT | No |
| `@lezer/json` | `1.0.3` | MIT | No |
| `@lezer/lr` | `1.4.10` | MIT | No |
| `@lezer/markdown` | `1.7.2` | MIT | No |
| `@lobehub/icons` | `5.16.0` | MIT | No |
| `@marijn/find-cluster-break` | `1.0.4` | MIT | No |
| `@mariozechner/clipboard` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-darwin-arm64` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-darwin-universal` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-darwin-x64` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-linux-arm64-gnu` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-linux-arm64-musl` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-linux-riscv64-gnu` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-linux-x64-gnu` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-linux-x64-musl` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-win32-arm64-msvc` | `0.3.9` | MIT | Yes |
| `@mariozechner/clipboard-win32-x64-msvc` | `0.3.9` | MIT | Yes |
| `@mermaid-js/parser` | `1.2.0` | MIT | No |
| `@modelcontextprotocol/sdk` | `1.30.0` | MIT | No |
| `@next/env` | `16.3.3` | MIT | No |
| `@next/swc-darwin-arm64` | `16.3.3` | MIT | Yes |
| `@next/swc-darwin-x64` | `16.3.3` | MIT | Yes |
| `@next/swc-linux-arm64-gnu` | `16.3.3` | MIT | Yes |
| `@next/swc-linux-arm64-musl` | `16.3.3` | MIT | Yes |
| `@next/swc-linux-x64-gnu` | `16.3.3` | MIT | Yes |
| `@next/swc-linux-x64-musl` | `16.3.3` | MIT | Yes |
| `@next/swc-win32-arm64-msvc` | `16.3.3` | MIT | Yes |
| `@next/swc-win32-x64-msvc` | `16.3.3` | MIT | Yes |
| `@nodable/entities` | `2.1.0` | MIT | No |
| `@oozcitak/dom` | `1.15.10` | MIT | No |
| `@oozcitak/infra` | `1.0.8` | MIT | No |
| `@oozcitak/url` | `1.0.4` | MIT | No |
| `@oozcitak/util` | `8.3.8` | MIT | No |
| `@pkgjs/parseargs` | `0.11.0` | MIT | Yes |
| `@protobufjs/aspromise` | `1.1.2` | BSD-3-Clause | No |
| `@protobufjs/base64` | `1.1.2` | BSD-3-Clause | No |
| `@protobufjs/codegen` | `2.0.5` | BSD-3-Clause | No |
| `@protobufjs/eventemitter` | `1.1.1` | BSD-3-Clause | No |
| `@protobufjs/fetch` | `1.1.1` | BSD-3-Clause | No |
| `@protobufjs/float` | `1.0.2` | BSD-3-Clause | No |
| `@protobufjs/path` | `1.1.2` | BSD-3-Clause | No |
| `@protobufjs/pool` | `1.1.0` | BSD-3-Clause | No |
| `@protobufjs/utf8` | `1.1.1` | BSD-3-Clause | No |
| `@protobufjs/utf8` | `1.1.2` | BSD-3-Clause | No |
| `@rc-component/util` | `1.10.1` | MIT | No |
| `@silvia-odwyer/photon-node` | `0.3.4` | Apache-2.0 | No |
| `@smithy/core` | `3.24.3` | Apache-2.0 | No |
| `@smithy/core` | `3.33.3` | Apache-2.0 | No |
| `@smithy/credential-provider-imds` | `4.3.3` | Apache-2.0 | No |
| `@smithy/credential-provider-imds` | `4.5.2` | Apache-2.0 | No |
| `@smithy/fetch-http-handler` | `5.4.3` | Apache-2.0 | No |
| `@smithy/fetch-http-handler` | `5.7.2` | Apache-2.0 | No |
| `@smithy/is-array-buffer` | `2.2.0` | Apache-2.0 | No |
| `@smithy/node-http-handler` | `4.11.3` | Apache-2.0 | No |
| `@smithy/node-http-handler` | `4.7.3` | Apache-2.0 | No |
| `@smithy/signature-v4` | `5.4.3` | Apache-2.0 | No |
| `@smithy/signature-v4` | `5.7.3` | Apache-2.0 | No |
| `@smithy/types` | `4.14.2` | Apache-2.0 | No |
| `@smithy/types` | `4.17.2` | Apache-2.0 | No |
| `@smithy/util-buffer-from` | `2.2.0` | Apache-2.0 | No |
| `@smithy/util-utf8` | `2.3.0` | Apache-2.0 | No |
| `@swc/helpers` | `0.5.23` | Apache-2.0 | No |
| `@types/d3` | `7.4.3` | MIT | No |
| `@types/d3-array` | `3.2.2` | MIT | No |
| `@types/d3-axis` | `3.0.6` | MIT | No |
| `@types/d3-brush` | `3.0.6` | MIT | No |
| `@types/d3-chord` | `3.0.6` | MIT | No |
| `@types/d3-color` | `3.1.3` | MIT | No |
| `@types/d3-contour` | `3.0.6` | MIT | No |
| `@types/d3-delaunay` | `6.0.4` | MIT | No |
| `@types/d3-dispatch` | `3.0.7` | MIT | No |
| `@types/d3-drag` | `3.0.7` | MIT | No |
| `@types/d3-dsv` | `3.0.7` | MIT | No |
| `@types/d3-ease` | `3.0.2` | MIT | No |
| `@types/d3-fetch` | `3.0.7` | MIT | No |
| `@types/d3-force` | `3.0.10` | MIT | No |
| `@types/d3-format` | `3.0.4` | MIT | No |
| `@types/d3-geo` | `3.1.0` | MIT | No |
| `@types/d3-hierarchy` | `3.1.7` | MIT | No |
| `@types/d3-interpolate` | `3.0.4` | MIT | No |
| `@types/d3-path` | `3.1.1` | MIT | No |
| `@types/d3-polygon` | `3.0.2` | MIT | No |
| `@types/d3-quadtree` | `3.0.6` | MIT | No |
| `@types/d3-random` | `3.0.3` | MIT | No |
| `@types/d3-scale` | `4.0.9` | MIT | No |
| `@types/d3-scale-chromatic` | `3.1.0` | MIT | No |
| `@types/d3-selection` | `3.0.11` | MIT | No |
| `@types/d3-shape` | `3.1.8` | MIT | No |
| `@types/d3-time` | `3.0.4` | MIT | No |
| `@types/d3-time-format` | `4.0.3` | MIT | No |
| `@types/d3-timer` | `3.0.2` | MIT | No |
| `@types/d3-transition` | `3.0.9` | MIT | No |
| `@types/d3-zoom` | `3.0.8` | MIT | No |
| `@types/debug` | `4.1.13` | MIT | No |
| `@types/estree` | `1.0.8` | MIT | No |
| `@types/estree-jsx` | `1.0.5` | MIT | No |
| `@types/geojson` | `7946.0.16` | MIT | No |
| `@types/hast` | `3.0.4` | MIT | No |
| `@types/katex` | `0.16.8` | MIT | No |
| `@types/mdast` | `4.0.4` | MIT | No |
| `@types/ms` | `2.1.0` | MIT | No |
| `@types/node` | `22.19.19` | MIT | No |
| `@types/node` | `25.9.5` | MIT | No |
| `@types/parse-json` | `4.0.2` | MIT | No |
| `@types/prismjs` | `1.26.6` | MIT | No |
| `@types/retry` | `0.12.0` | MIT | No |
| `@types/trusted-types` | `2.0.7` | MIT | Yes |
| `@types/unist` | `2.0.11` | MIT | No |
| `@types/unist` | `3.0.3` | MIT | No |
| `@ungap/structured-clone` | `1.3.0` | ISC | No |
| `@upsetjs/venn.js` | `2.0.0` | MIT | No |
| `@xmldom/xmldom` | `0.8.15` | MIT | No |
| `@xterm/addon-fit` | `0.11.0` | MIT | No |
| `@xterm/addon-search` | `0.16.0` | MIT | No |
| `@xterm/xterm` | `6.0.0` | MIT | No |
| `abort-controller` | `3.0.0` | MIT | No |
| `accepts` | `2.0.0` | MIT | No |
| `acorn` | `8.16.0` | MIT | No |
| `agent-base` | `6.0.2` | MIT | No |
| `agent-base` | `7.1.4` | MIT | No |
| `ajv` | `8.20.0` | MIT | No |
| `ajv-formats` | `3.0.1` | MIT | No |
| `ansi-regex` | `5.0.1` | MIT | No |
| `ansi-regex` | `6.3.0` | MIT | No |
| `ansi-styles` | `4.3.0` | MIT | No |
| `ansi-styles` | `6.2.3` | MIT | No |
| `antd-style` | `4.1.0` | MIT | No |
| `archiver` | `7.0.1` | MIT | No |
| `archiver-utils` | `5.0.2` | MIT | No |
| `argparse` | `1.0.10` | MIT | No |
| `argparse` | `2.0.1` | Python-2.0 | No |
| `async` | `3.2.6` | MIT | No |
| `asyncbox` | `3.0.0` | Apache-2.0 | No |
| `asynckit` | `0.4.0` | MIT | No |
| `axios` | `1.20.0` | MIT | No |
| `b4a` | `1.8.1` | Apache-2.0 | No |
| `babel-plugin-macros` | `3.1.0` | MIT | No |
| `bail` | `2.0.2` | MIT | No |
| `balanced-match` | `1.0.2` | MIT | No |
| `balanced-match` | `4.0.4` | MIT | No |
| `bare-events` | `2.9.2` | Apache-2.0 | No |
| `bare-fs` | `4.8.1` | Apache-2.0 | No |
| `bare-path` | `3.1.2` | Apache-2.0 | No |
| `bare-stream` | `2.13.4` | Apache-2.0 | No |
| `bare-url` | `2.5.3` | Apache-2.0 | No |
| `base64-js` | `1.5.1` | MIT | No |
| `baseline-browser-mapping` | `2.11.9` | Apache-2.0 | No |
| `bignumber.js` | `9.3.1` | MIT | No |
| `bluebird` | `3.4.7` | MIT | No |
| `bluebird` | `3.7.2` | MIT | No |
| `body-parser` | `2.3.0` | MIT | No |
| `bowser` | `2.14.1` | MIT | No |
| `brace-expansion` | `2.1.4` | MIT | No |
| `brace-expansion` | `5.0.9` | MIT | No |
| `buffer` | `6.0.3` | MIT | No |
| `buffer-crc32` | `1.0.0` | MIT | No |
| `buffer-equal-constant-time` | `1.0.1` | BSD-3-Clause | No |
| `buffer-from` | `1.1.2` | MIT | No |
| `builder-util-runtime` | `9.7.0` | MIT | No |
| `bytes` | `3.1.2` | MIT | No |
| `call-bind-apply-helpers` | `1.0.2` | MIT | No |
| `call-bound` | `1.0.4` | MIT | No |
| `callsites` | `3.1.0` | MIT | No |
| `caniuse-lite` | `1.0.30001806` | CC-BY-4.0 | No |
| `ccount` | `2.0.1` | MIT | No |
| `chalk` | `5.6.2` | MIT | No |
| `character-entities` | `2.0.2` | MIT | No |
| `character-entities-html4` | `2.1.0` | MIT | No |
| `character-entities-legacy` | `3.0.0` | MIT | No |
| `character-reference-invalid` | `2.0.1` | MIT | No |
| `chownr` | `3.0.0` | BlueOak-1.0.0 | No |
| `client-only` | `0.0.1` | MIT | No |
| `cliui` | `8.0.1` | ISC | No |
| `clsx` | `2.1.1` | MIT | No |
| `codemirror` | `6.0.2` | MIT | No |
| `color-convert` | `2.0.1` | MIT | No |
| `color-name` | `1.1.4` | MIT | No |
| `combined-stream` | `1.0.8` | MIT | No |
| `comma-separated-tokens` | `2.0.3` | MIT | No |
| `commander` | `7.2.0` | MIT | No |
| `commander` | `8.3.0` | MIT | No |
| `compare-versions` | `6.1.1` | MIT | No |
| `compress-commons` | `6.0.2` | MIT | No |
| `confbox` | `0.1.8` | MIT | No |
| `content-disposition` | `1.1.0` | MIT | No |
| `content-type` | `1.0.5` | MIT | No |
| `content-type` | `2.1.0` | MIT | No |
| `convert-source-map` | `1.9.0` | MIT | No |
| `cookie` | `0.7.2` | MIT | No |
| `cookie-signature` | `1.2.2` | MIT | No |
| `core-util-is` | `1.0.3` | MIT | No |
| `cors` | `2.8.6` | MIT | No |
| `cose-base` | `1.0.3` | MIT | No |
| `cose-base` | `2.2.0` | MIT | No |
| `cosmiconfig` | `7.1.0` | MIT | No |
| `crc-32` | `1.2.2` | Apache-2.0 | No |
| `crc32-stream` | `6.0.0` | MIT | No |
| `crelt` | `1.0.7` | MIT | No |
| `cross-spawn` | `7.0.6` | MIT | No |
| `csstype` | `3.2.3` | MIT | No |
| `cytoscape` | `3.33.3` | MIT | No |
| `cytoscape-cose-bilkent` | `4.1.0` | MIT | No |
| `cytoscape-fcose` | `2.2.0` | MIT | No |
| `d3` | `7.9.0` | ISC | No |
| `d3-array` | `2.12.1` | BSD-3-Clause | No |
| `d3-array` | `3.2.4` | ISC | No |
| `d3-axis` | `3.0.0` | ISC | No |
| `d3-brush` | `3.0.0` | ISC | No |
| `d3-chord` | `3.0.1` | ISC | No |
| `d3-color` | `3.1.0` | ISC | No |
| `d3-contour` | `4.0.2` | ISC | No |
| `d3-delaunay` | `6.0.4` | ISC | No |
| `d3-dispatch` | `3.0.1` | ISC | No |
| `d3-drag` | `3.0.0` | ISC | No |
| `d3-dsv` | `3.0.1` | ISC | No |
| `d3-ease` | `3.0.1` | BSD-3-Clause | No |
| `d3-fetch` | `3.0.1` | ISC | No |
| `d3-force` | `3.0.0` | ISC | No |
| `d3-format` | `3.1.2` | ISC | No |
| `d3-geo` | `3.1.1` | ISC | No |
| `d3-hierarchy` | `3.1.2` | ISC | No |
| `d3-interpolate` | `3.0.1` | ISC | No |
| `d3-path` | `1.0.9` | BSD-3-Clause | No |
| `d3-path` | `3.1.0` | ISC | No |
| `d3-polygon` | `3.0.1` | ISC | No |
| `d3-quadtree` | `3.0.1` | ISC | No |
| `d3-random` | `3.0.1` | ISC | No |
| `d3-sankey` | `0.12.3` | BSD-3-Clause | No |
| `d3-scale` | `4.0.2` | ISC | No |
| `d3-scale-chromatic` | `3.1.0` | ISC | No |
| `d3-selection` | `3.0.0` | ISC | No |
| `d3-shape` | `1.3.7` | BSD-3-Clause | No |
| `d3-shape` | `3.2.0` | ISC | No |
| `d3-time` | `3.1.0` | ISC | No |
| `d3-time-format` | `4.1.0` | ISC | No |
| `d3-timer` | `3.0.1` | ISC | No |
| `d3-transition` | `3.0.1` | ISC | No |
| `d3-zoom` | `3.0.0` | ISC | No |
| `dagre-d3-es` | `7.0.14` | MIT | No |
| `data-uri-to-buffer` | `4.0.1` | MIT | No |
| `dayjs` | `1.11.20` | MIT | No |
| `debug` | `4.4.3` | MIT | No |
| `decode-named-character-reference` | `1.3.0` | MIT | No |
| `delaunator` | `5.1.0` | ISC | No |
| `delayed-stream` | `1.0.0` | MIT | No |
| `depd` | `2.0.0` | MIT | No |
| `dequal` | `2.0.3` | MIT | No |
| `detect-libc` | `2.1.2` | Apache-2.0 | No |
| `devlop` | `1.1.0` | MIT | No |
| `diff` | `8.0.4` | BSD-3-Clause | No |
| `diff-match-patch` | `1.0.5` | Apache-2.0 | No |
| `dingbat-to-unicode` | `1.0.1` | BSD-2-Clause | No |
| `dompurify` | `3.4.13` | (MPL-2.0 OR Apache-2.0) | No |
| `duck` | `0.1.12` | BSD | No |
| `dunder-proto` | `1.0.1` | MIT | No |
| `eastasianwidth` | `0.2.0` | MIT | No |
| `ecdsa-sig-formatter` | `1.0.11` | Apache-2.0 | No |
| `ee-first` | `1.1.1` | MIT | No |
| `electron-updater` | `6.8.9` | MIT | No |
| `emoji-regex` | `8.0.0` | MIT | No |
| `emoji-regex` | `9.2.2` | MIT | No |
| `encodeurl` | `2.0.0` | MIT | No |
| `entities` | `6.0.1` | BSD-2-Clause | No |
| `error-ex` | `1.3.4` | MIT | No |
| `es-define-property` | `1.0.1` | MIT | No |
| `es-errors` | `1.3.0` | MIT | No |
| `es-object-atoms` | `1.1.1` | MIT | No |
| `es-set-tostringtag` | `2.1.0` | MIT | No |
| `es-toolkit` | `1.50.0` | MIT | No |
| `escalade` | `3.2.0` | MIT | No |
| `escape-html` | `1.0.3` | MIT | No |
| `escape-string-regexp` | `4.0.0` | MIT | No |
| `escape-string-regexp` | `5.0.0` | MIT | No |
| `estree-util-is-identifier-name` | `3.0.0` | MIT | No |
| `etag` | `1.8.1` | MIT | No |
| `event-target-shim` | `5.0.1` | MIT | No |
| `events` | `3.3.0` | MIT | No |
| `events-universal` | `1.0.1` | Apache-2.0 | No |
| `eventsource` | `3.0.7` | MIT | No |
| `eventsource-parser` | `3.1.1` | MIT | No |
| `express` | `5.2.1` | MIT | No |
| `express-rate-limit` | `8.7.0` | MIT | No |
| `extend` | `3.0.2` | MIT | No |
| `fast-deep-equal` | `3.1.3` | MIT | No |
| `fast-fifo` | `1.3.2` | MIT | No |
| `fast-uri` | `3.1.7` | BSD-3-Clause | No |
| `fast-xml-builder` | `1.2.0` | MIT | No |
| `fast-xml-parser` | `5.7.3` | MIT | No |
| `fault` | `1.0.4` | MIT | No |
| `fetch-blob` | `3.2.0` | MIT | No |
| `finalhandler` | `2.1.1` | MIT | No |
| `find-root` | `1.1.0` | MIT | No |
| `follow-redirects` | `1.16.0` | MIT | No |
| `foreground-child` | `3.3.1` | ISC | No |
| `form-data` | `4.0.6` | MIT | No |
| `format` | `0.2.2` | MIT | No |
| `formdata-polyfill` | `4.0.10` | MIT | No |
| `forwarded` | `0.2.0` | MIT | No |
| `fresh` | `2.0.0` | MIT | No |
| `fs-extra` | `10.1.0` | MIT | No |
| `function-bind` | `1.1.2` | MIT | No |
| `gaxios` | `7.1.4` | Apache-2.0 | No |
| `gaxios` | `7.3.1` | Apache-2.0 | No |
| `gcp-metadata` | `8.1.2` | Apache-2.0 | No |
| `get-caller-file` | `2.0.5` | ISC | No |
| `get-east-asian-width` | `1.6.0` | MIT | No |
| `get-intrinsic` | `1.3.0` | MIT | No |
| `get-proto` | `1.0.1` | MIT | No |
| `glob` | `10.5.0` | ISC | No |
| `google-auth-library` | `10.6.2` | Apache-2.0 | No |
| `google-auth-library` | `10.9.1` | Apache-2.0 | No |
| `google-logging-utils` | `1.1.3` | Apache-2.0 | No |
| `google-protobuf` | `3.21.4` | (BSD-3-Clause AND Apache-2.0) | No |
| `gopd` | `1.2.0` | MIT | No |
| `graceful-fs` | `4.2.11` | ISC | No |
| `grok-mermaid` | `0.2.2` | Apache-2.0 | No |
| `hachure-fill` | `0.5.2` | MIT | No |
| `has-symbols` | `1.1.0` | MIT | No |
| `has-tostringtag` | `1.0.2` | MIT | No |
| `hasown` | `2.0.2` | MIT | No |
| `hasown` | `2.0.4` | MIT | No |
| `hast-util-from-dom` | `5.0.1` | ISC | No |
| `hast-util-from-html` | `2.0.3` | MIT | No |
| `hast-util-from-html-isomorphic` | `2.0.0` | MIT | No |
| `hast-util-from-parse5` | `8.0.3` | MIT | No |
| `hast-util-is-element` | `3.0.0` | MIT | No |
| `hast-util-parse-selector` | `4.0.0` | MIT | No |
| `hast-util-raw` | `9.1.0` | MIT | No |
| `hast-util-sanitize` | `5.0.2` | MIT | No |
| `hast-util-to-jsx-runtime` | `2.3.6` | MIT | No |
| `hast-util-to-parse5` | `8.0.1` | MIT | No |
| `hast-util-to-text` | `4.0.2` | MIT | No |
| `hast-util-whitespace` | `3.0.0` | MIT | No |
| `hastscript` | `9.0.1` | MIT | No |
| `highlight.js` | `10.7.3` | BSD-3-Clause | No |
| `highlightjs-vue` | `1.0.0` | CC0-1.0 | No |
| `hoist-non-react-statics` | `3.3.2` | BSD-3-Clause | No |
| `hono` | `4.13.7` | MIT | No |
| `hosted-git-info` | `9.0.3` | ISC | No |
| `html-url-attributes` | `3.0.1` | MIT | No |
| `html-void-elements` | `3.0.0` | MIT | No |
| `http-errors` | `2.0.1` | MIT | No |
| `http-proxy-agent` | `7.0.2` | MIT | No |
| `https-proxy-agent` | `5.0.1` | MIT | No |
| `https-proxy-agent` | `7.0.6` | MIT | No |
| `hypium-driver` | `6.1.210` | ISC | No |
| `iconv-lite` | `0.6.3` | MIT | No |
| `iconv-lite` | `0.7.3` | MIT | No |
| `ieee754` | `1.2.1` | BSD-3-Clause | No |
| `ignore` | `7.0.5` | MIT | No |
| `immediate` | `3.0.6` | MIT | No |
| `import-fresh` | `3.3.1` | MIT | No |
| `inherits` | `2.0.4` | ISC | No |
| `inline-style-parser` | `0.2.7` | MIT | No |
| `internmap` | `1.0.1` | ISC | No |
| `internmap` | `2.0.3` | ISC | No |
| `ip-address` | `10.7.0` | MIT | No |
| `ipaddr.js` | `1.9.1` | MIT | No |
| `is-alphabetical` | `2.0.1` | MIT | No |
| `is-alphanumerical` | `2.0.1` | MIT | No |
| `is-arrayish` | `0.2.1` | MIT | No |
| `is-core-module` | `2.16.1` | MIT | No |
| `is-decimal` | `2.0.1` | MIT | No |
| `is-fullwidth-code-point` | `3.0.0` | MIT | No |
| `is-hexadecimal` | `2.0.1` | MIT | No |
| `is-mobile` | `5.0.0` | MIT | No |
| `is-plain-obj` | `4.1.0` | MIT | No |
| `is-promise` | `4.0.0` | MIT | No |
| `is-stream` | `2.0.1` | MIT | No |
| `isarray` | `1.0.0` | MIT | No |
| `isexe` | `2.0.0` | ISC | No |
| `jackspeak` | `3.4.3` | BlueOak-1.0.0 | No |
| `jiti` | `2.7.0` | MIT | No |
| `jose` | `6.2.12` | MIT | No |
| `js-tokens` | `4.0.0` | MIT | No |
| `js-yaml` | `4.3.2` | MIT | No |
| `jsesc` | `3.1.0` | MIT | No |
| `json-bigint` | `1.0.0` | MIT | No |
| `json-parse-even-better-errors` | `2.3.1` | MIT | No |
| `json-schema-to-ts` | `3.1.1` | MIT | No |
| `json-schema-traverse` | `1.0.0` | MIT | No |
| `json-schema-typed` | `8.0.2` | BSD-2-Clause | No |
| `json5` | `2.2.3` | MIT | No |
| `jsonfile` | `6.2.1` | MIT | No |
| `jszip` | `3.10.1` | (MIT OR GPL-3.0-or-later) | No |
| `jwa` | `2.0.1` | MIT | No |
| `jws` | `4.0.1` | MIT | No |
| `katex` | `0.16.47` | MIT | No |
| `khroma` | `2.1.0` | MIT | No |
| `koffi` | `3.2.1` | MIT | No |
| `layout-base` | `1.0.2` | MIT | No |
| `layout-base` | `2.0.1` | MIT | No |
| `lazy-val` | `1.0.5` | MIT | No |
| `lazystream` | `1.0.1` | MIT | No |
| `lie` | `3.3.0` | MIT | No |
| `lines-and-columns` | `1.2.4` | MIT | No |
| `lodash` | `4.18.1` | MIT | No |
| `lodash-es` | `4.18.1` | MIT | No |
| `lodash.camelcase` | `4.3.0` | MIT | No |
| `lodash.escaperegexp` | `4.1.2` | MIT | No |
| `lodash.isequal` | `4.5.0` | MIT | No |
| `long` | `5.3.2` | Apache-2.0 | No |
| `longest-streak` | `3.1.0` | MIT | No |
| `lop` | `0.4.2` | BSD-2-Clause | No |
| `lowlight` | `1.20.0` | MIT | No |
| `lru-cache` | `10.4.3` | ISC | No |
| `lru-cache` | `11.4.0` | BlueOak-1.0.0 | No |
| `lucide-react` | `0.469.0` | ISC | No |
| `mammoth` | `1.12.1` | BSD-2-Clause | No |
| `markdown-table` | `3.0.4` | MIT | No |
| `marked` | `16.4.2` | MIT | No |
| `marked` | `18.0.5` | MIT | No |
| `math-intrinsics` | `1.1.0` | MIT | No |
| `mdast-util-find-and-replace` | `3.0.2` | MIT | No |
| `mdast-util-from-markdown` | `2.0.3` | MIT | No |
| `mdast-util-gfm` | `3.1.0` | MIT | No |
| `mdast-util-gfm-autolink-literal` | `2.0.1` | MIT | No |
| `mdast-util-gfm-footnote` | `2.1.0` | MIT | No |
| `mdast-util-gfm-strikethrough` | `2.0.0` | MIT | No |
| `mdast-util-gfm-table` | `2.0.0` | MIT | No |
| `mdast-util-gfm-task-list-item` | `2.0.0` | MIT | No |
| `mdast-util-math` | `3.0.0` | MIT | No |
| `mdast-util-mdx-expression` | `2.0.1` | MIT | No |
| `mdast-util-mdx-jsx` | `3.2.0` | MIT | No |
| `mdast-util-mdxjs-esm` | `2.0.1` | MIT | No |
| `mdast-util-phrasing` | `4.1.0` | MIT | No |
| `mdast-util-to-hast` | `13.2.1` | MIT | No |
| `mdast-util-to-markdown` | `2.1.2` | MIT | No |
| `mdast-util-to-string` | `4.0.0` | MIT | No |
| `media-typer` | `1.1.1` | MIT | No |
| `merge-descriptors` | `2.0.0` | MIT | No |
| `mermaid` | `11.16.1` | MIT | No |
| `micromark` | `4.0.2` | MIT | No |
| `micromark-core-commonmark` | `2.0.3` | MIT | No |
| `micromark-extension-gfm` | `3.0.0` | MIT | No |
| `micromark-extension-gfm-autolink-literal` | `2.1.0` | MIT | No |
| `micromark-extension-gfm-footnote` | `2.1.0` | MIT | No |
| `micromark-extension-gfm-strikethrough` | `2.1.0` | MIT | No |
| `micromark-extension-gfm-table` | `2.1.1` | MIT | No |
| `micromark-extension-gfm-tagfilter` | `2.0.0` | MIT | No |
| `micromark-extension-gfm-task-list-item` | `2.1.0` | MIT | No |
| `micromark-extension-math` | `3.1.0` | MIT | No |
| `micromark-factory-destination` | `2.0.1` | MIT | No |
| `micromark-factory-label` | `2.0.1` | MIT | No |
| `micromark-factory-space` | `2.0.1` | MIT | No |
| `micromark-factory-title` | `2.0.1` | MIT | No |
| `micromark-factory-whitespace` | `2.0.1` | MIT | No |
| `micromark-util-character` | `2.1.1` | MIT | No |
| `micromark-util-chunked` | `2.0.1` | MIT | No |
| `micromark-util-classify-character` | `2.0.1` | MIT | No |
| `micromark-util-combine-extensions` | `2.0.1` | MIT | No |
| `micromark-util-decode-numeric-character-reference` | `2.0.2` | MIT | No |
| `micromark-util-decode-string` | `2.0.1` | MIT | No |
| `micromark-util-encode` | `2.0.1` | MIT | No |
| `micromark-util-html-tag-name` | `2.0.1` | MIT | No |
| `micromark-util-normalize-identifier` | `2.0.1` | MIT | No |
| `micromark-util-resolve-all` | `2.0.1` | MIT | No |
| `micromark-util-sanitize-uri` | `2.0.1` | MIT | No |
| `micromark-util-subtokenize` | `2.1.0` | MIT | No |
| `micromark-util-symbol` | `2.0.1` | MIT | No |
| `micromark-util-types` | `2.0.2` | MIT | No |
| `mime-db` | `1.52.0` | MIT | No |
| `mime-db` | `1.54.0` | MIT | No |
| `mime-types` | `2.1.35` | MIT | No |
| `mime-types` | `3.0.2` | MIT | No |
| `minimatch` | `10.2.5` | BlueOak-1.0.0 | No |
| `minimatch` | `5.1.9` | ISC | No |
| `minimatch` | `9.0.9` | ISC | No |
| `minipass` | `7.1.3` | BlueOak-1.0.0 | No |
| `minizlib` | `3.1.0` | MIT | No |
| `mlly` | `1.8.2` | MIT | No |
| `moment` | `2.30.1` | MIT | No |
| `ms` | `2.1.3` | MIT | No |
| `nanoid` | `3.3.18` | MIT | No |
| `negotiator` | `1.1.0` | MIT | No |
| `next` | `16.3.3` | MIT | No |
| `node-addon-api` | `7.1.1` | MIT | No |
| `node-domexception` | `1.0.0` | MIT | No |
| `node-fetch` | `3.3.2` | MIT | No |
| `node-pty` | `1.1.0` | MIT | No |
| `normalize-path` | `3.0.0` | MIT | No |
| `object-assign` | `4.1.1` | MIT | No |
| `object-inspect` | `1.13.4` | MIT | No |
| `on-finished` | `2.4.1` | MIT | No |
| `once` | `1.4.0` | ISC | No |
| `openai` | `6.40.0` | Apache-2.0 | No |
| `opencc-js` | `1.4.1` | MIT AND Apache-2.0 | No |
| `option` | `0.2.4` | BSD-2-Clause | No |
| `p-retry` | `4.6.2` | MIT | No |
| `package-json-from-dist` | `1.0.1` | BlueOak-1.0.0 | No |
| `package-manager-detector` | `1.6.0` | MIT | No |
| `pako` | `1.0.11` | (MIT AND Zlib) | No |
| `parent-module` | `1.0.1` | MIT | No |
| `parse-entities` | `4.0.2` | MIT | No |
| `parse-json` | `5.2.0` | MIT | No |
| `parse5` | `7.3.0` | MIT | No |
| `parseurl` | `1.3.3` | MIT | No |
| `partial-json` | `0.1.7` | MIT | No |
| `path-data-parser` | `0.1.0` | MIT | No |
| `path-expression-matcher` | `1.5.0` | MIT | No |
| `path-is-absolute` | `1.0.1` | MIT | No |
| `path-key` | `3.1.1` | MIT | No |
| `path-parse` | `1.0.7` | MIT | No |
| `path-scurry` | `1.11.1` | BlueOak-1.0.0 | No |
| `path-to-regexp` | `8.4.2` | MIT | No |
| `path-type` | `4.0.0` | MIT | No |
| `pathe` | `2.0.3` | MIT | No |
| `picocolors` | `1.1.1` | ISC | No |
| `pkce-challenge` | `5.0.1` | MIT | No |
| `pkg-types` | `1.3.1` | MIT | No |
| `playwright-core` | `1.62.1` | Apache-2.0 | No |
| `points-on-curve` | `0.2.0` | MIT | No |
| `points-on-path` | `0.2.1` | MIT | No |
| `polished` | `4.3.1` | MIT | No |
| `portfinder` | `1.0.38` | MIT | No |
| `postcss` | `8.5.26` | MIT | No |
| `prismjs` | `1.30.0` | MIT | No |
| `process` | `0.11.10` | MIT | No |
| `process-nextick-args` | `2.0.1` | MIT | No |
| `proper-lockfile` | `4.1.2` | MIT | No |
| `property-information` | `7.1.0` | MIT | No |
| `protobufjs` | `7.6.5` | BSD-3-Clause | No |
| `protobufjs` | `7.6.6` | BSD-3-Clause | No |
| `proxy-addr` | `2.0.7` | MIT | No |
| `proxy-from-env` | `2.1.0` | MIT | No |
| `qs` | `6.16.0` | BSD-3-Clause | No |
| `range-parser` | `1.3.0` | MIT | No |
| `raw-body` | `3.0.2` | MIT | No |
| `react` | `19.2.8` | MIT | No |
| `react-dom` | `19.2.8` | MIT | No |
| `react-is` | `16.13.1` | MIT | No |
| `react-is` | `18.3.1` | MIT | No |
| `react-markdown` | `10.1.0` | MIT | No |
| `react-syntax-highlighter` | `16.1.1` | MIT | No |
| `readable-stream` | `2.3.8` | MIT | No |
| `readable-stream` | `4.7.0` | MIT | No |
| `readdir-glob` | `1.1.3` | Apache-2.0 | No |
| `refractor` | `5.0.0` | MIT | No |
| `rehype-katex` | `7.0.1` | MIT | No |
| `rehype-raw` | `7.0.0` | MIT | No |
| `rehype-sanitize` | `6.0.0` | MIT | No |
| `remark-gfm` | `4.0.1` | MIT | No |
| `remark-math` | `6.0.0` | MIT | No |
| `remark-parse` | `11.0.0` | MIT | No |
| `remark-rehype` | `11.1.2` | MIT | No |
| `remark-stringify` | `11.0.0` | MIT | No |
| `require-directory` | `2.1.1` | MIT | No |
| `require-from-string` | `2.0.2` | MIT | No |
| `resolve` | `1.22.11` | MIT | No |
| `resolve-from` | `4.0.0` | MIT | No |
| `retry` | `0.12.0` | MIT | No |
| `retry` | `0.13.1` | MIT | No |
| `robust-predicates` | `3.0.3` | Unlicense | No |
| `roughjs` | `4.6.6` | MIT | No |
| `router` | `2.2.0` | MIT | No |
| `rrule` | `2.8.1` | BSD-3-Clause | No |
| `rw` | `1.3.3` | BSD-3-Clause | No |
| `safe-buffer` | `5.1.2` | MIT | No |
| `safe-buffer` | `5.2.1` | MIT | No |
| `safer-buffer` | `2.1.2` | MIT | No |
| `sax` | `1.6.1` | BlueOak-1.0.0 | No |
| `scheduler` | `0.27.0` | MIT | No |
| `semver` | `7.7.4` | ISC | No |
| `semver` | `7.8.0` | ISC | No |
| `semver` | `7.8.5` | ISC | No |
| `send` | `1.2.1` | MIT | No |
| `serve-static` | `2.2.1` | MIT | No |
| `setimmediate` | `1.0.5` | MIT | No |
| `setprototypeof` | `1.2.0` | ISC | No |
| `sharp` | `0.35.4` | Apache-2.0 | No |
| `shebang-command` | `2.0.0` | MIT | No |
| `shebang-regex` | `3.0.0` | MIT | No |
| `shell-quote` | `1.10.0` | MIT | No |
| `shlex` | `2.1.2` | MIT | No |
| `side-channel` | `1.1.1` | MIT | No |
| `side-channel-list` | `1.0.1` | MIT | No |
| `side-channel-map` | `1.0.1` | MIT | No |
| `side-channel-weakmap` | `1.0.2` | MIT | No |
| `signal-exit` | `3.0.7` | ISC | No |
| `signal-exit` | `4.1.0` | ISC | No |
| `source-map` | `0.5.7` | BSD-3-Clause | No |
| `source-map` | `0.6.1` | BSD-3-Clause | No |
| `source-map-js` | `1.2.1` | BSD-3-Clause | No |
| `source-map-support` | `0.5.21` | MIT | No |
| `space-separated-tokens` | `2.0.2` | MIT | No |
| `sprintf-js` | `1.0.3` | BSD-3-Clause | No |
| `statuses` | `2.0.2` | MIT | No |
| `streamx` | `2.28.1` | MIT | No |
| `string-width` | `4.2.3` | MIT | No |
| `string-width` | `5.1.2` | MIT | No |
| `string-width-cjs` | `4.2.3` | MIT | No |
| `string_decoder` | `1.1.1` | MIT | No |
| `string_decoder` | `1.3.0` | MIT | No |
| `stringify-entities` | `4.0.4` | MIT | No |
| `strip-ansi` | `6.0.1` | MIT | No |
| `strip-ansi` | `7.2.0` | MIT | No |
| `strip-ansi-cjs` | `6.0.1` | MIT | No |
| `strnum` | `2.3.0` | MIT | No |
| `style-mod` | `4.1.3` | MIT | No |
| `style-to-js` | `1.1.21` | MIT | No |
| `style-to-object` | `1.0.14` | MIT | No |
| `styled-jsx` | `5.1.6` | MIT | No |
| `stylis` | `4.2.0` | MIT | No |
| `stylis` | `4.4.0` | MIT | No |
| `supports-preserve-symlinks-flag` | `1.0.0` | MIT | No |
| `tar` | `7.5.22` | BlueOak-1.0.0 | No |
| `tar-stream` | `3.2.1` | MIT | No |
| `teen_process` | `2.3.3` | Apache-2.0 | No |
| `teex` | `1.0.1` | MIT | No |
| `text-decoder` | `1.2.7` | Apache-2.0 | No |
| `three` | `0.185.1` | MIT | No |
| `tiny-typed-emitter` | `2.1.0` | MIT | No |
| `tinyexec` | `1.1.2` | MIT | No |
| `toidentifier` | `1.0.1` | MIT | No |
| `trim-lines` | `3.0.1` | MIT | No |
| `trough` | `2.2.0` | MIT | No |
| `ts-algebra` | `2.0.0` | MIT | No |
| `ts-dedent` | `2.2.0` | MIT | No |
| `tslib` | `2.8.1` | 0BSD | No |
| `type-is` | `2.1.0` | MIT | No |
| `typebox` | `1.3.7` | MIT | No |
| `ufo` | `1.6.4` | MIT | No |
| `underscore` | `1.13.8` | MIT | No |
| `undici` | `8.9.0` | MIT | No |
| `undici-types` | `6.21.0` | MIT | No |
| `undici-types` | `7.24.6` | MIT | No |
| `unified` | `11.0.5` | MIT | No |
| `unist-util-find-after` | `5.0.0` | MIT | No |
| `unist-util-is` | `6.0.1` | MIT | No |
| `unist-util-position` | `5.0.0` | MIT | No |
| `unist-util-remove-position` | `5.0.0` | MIT | No |
| `unist-util-stringify-position` | `4.0.0` | MIT | No |
| `unist-util-visit` | `5.1.0` | MIT | No |
| `unist-util-visit-parents` | `6.0.2` | MIT | No |
| `universalify` | `2.0.1` | MIT | No |
| `unpipe` | `1.0.0` | MIT | No |
| `use-merge-value` | `1.2.0` | MIT | No |
| `util-deprecate` | `1.0.2` | MIT | No |
| `uuid` | `11.1.1` | MIT | No |
| `vary` | `1.1.2` | MIT | No |
| `vditor` | `4.0.0` | MIT | No |
| `vfile` | `6.0.3` | MIT | No |
| `vfile-location` | `5.0.3` | MIT | No |
| `vfile-message` | `4.0.3` | MIT | No |
| `w3c-keyname` | `2.2.8` | MIT | No |
| `web-namespaces` | `2.0.1` | MIT | No |
| `web-streams-polyfill` | `3.3.3` | MIT | No |
| `which` | `2.0.2` | ISC | No |
| `wrap-ansi` | `7.0.0` | MIT | No |
| `wrap-ansi` | `8.1.0` | MIT | No |
| `wrap-ansi-cjs` | `7.0.0` | MIT | No |
| `wrappy` | `1.0.2` | ISC | No |
| `ws` | `8.21.0` | MIT | No |
| `ws` | `8.21.3` | MIT | No |
| `xml-naming` | `0.1.0` | MIT | No |
| `xmlbuilder` | `10.1.1` | MIT | No |
| `xmlbuilder2` | `3.1.1` | MIT | No |
| `xmldom` | `0.9.12` | MIT | No |
| `xpath` | `0.0.34` | MIT | No |
| `y18n` | `5.0.8` | ISC | No |
| `yallist` | `5.0.0` | BlueOak-1.0.0 | No |
| `yaml` | `1.10.3` | ISC | No |
| `yaml` | `2.9.0` | ISC | No |
| `yargs` | `17.7.3` | MIT | No |
| `yargs-parser` | `21.1.1` | ISC | No |
| `zip-stream` | `6.0.1` | MIT | No |
| `zod` | `4.3.6` | MIT | No |
| `zod-to-json-schema` | `3.25.2` | ISC | No |
| `zwitch` | `2.0.4` | MIT | No |

## Build and development dependency closure

| Package | Version | Declared license | Optional |
| --- | --- | --- | --- |
| `@alloc/quick-lru` | `5.2.0` | MIT | No |
| `@babel/compat-data` | `7.29.7` | MIT | No |
| `@babel/core` | `7.29.7` | MIT | No |
| `@babel/helper-compilation-targets` | `7.29.7` | MIT | No |
| `@babel/helper-module-transforms` | `7.29.7` | MIT | No |
| `@babel/helper-validator-option` | `7.29.7` | MIT | No |
| `@babel/helpers` | `7.29.7` | MIT | No |
| `@dimforge/rapier3d-compat` | `0.12.0` | Apache-2.0 | No |
| `@electron-internal/extract-zip` | `1.0.5` | BSD-2-Clause | No |
| `@electron/asar` | `3.4.1` | MIT | No |
| `@electron/fuses` | `1.8.0` | MIT | No |
| `@electron/get` | `3.1.0` | MIT | No |
| `@electron/get` | `5.1.0` | MIT | No |
| `@electron/notarize` | `2.5.0` | MIT | No |
| `@electron/osx-sign` | `1.3.3` | BSD-2-Clause | No |
| `@electron/rebuild` | `4.2.0` | MIT | No |
| `@electron/universal` | `2.0.3` | MIT | No |
| `@emnapi/core` | `1.11.1` | MIT | Yes |
| `@emnapi/core` | `1.9.1` | MIT | Yes |
| `@emnapi/runtime` | `1.11.1` | MIT | Yes |
| `@emnapi/wasi-threads` | `1.2.0` | MIT | Yes |
| `@emnapi/wasi-threads` | `1.2.2` | MIT | Yes |
| `@eslint-community/eslint-utils` | `4.9.1` | MIT | No |
| `@eslint-community/regexpp` | `4.12.2` | MIT | No |
| `@eslint/config-array` | `0.21.2` | Apache-2.0 | No |
| `@eslint/config-helpers` | `0.4.2` | Apache-2.0 | No |
| `@eslint/core` | `0.17.0` | Apache-2.0 | No |
| `@eslint/eslintrc` | `3.3.6` | MIT | No |
| `@eslint/js` | `9.39.5` | MIT | No |
| `@eslint/object-schema` | `2.1.7` | Apache-2.0 | No |
| `@eslint/plugin-kit` | `0.4.1` | Apache-2.0 | No |
| `@humanfs/core` | `0.19.1` | Apache-2.0 | No |
| `@humanfs/node` | `0.16.7` | Apache-2.0 | No |
| `@humanwhocodes/module-importer` | `1.0.1` | Apache-2.0 | No |
| `@humanwhocodes/retry` | `0.4.3` | Apache-2.0 | No |
| `@jridgewell/remapping` | `2.3.5` | MIT | No |
| `@malept/cross-spawn-promise` | `2.0.0` | Apache-2.0 | No |
| `@malept/flatpak-bundler` | `0.4.0` | MIT | No |
| `@napi-rs/wasm-runtime` | `0.2.12` | MIT | Yes |
| `@napi-rs/wasm-runtime` | `1.1.4` | MIT | Yes |
| `@next/eslint-plugin-next` | `16.3.3` | MIT | No |
| `@noble/hashes` | `1.4.0` | MIT | No |
| `@noble/hashes` | `2.2.0` | MIT | No |
| `@nodelib/fs.scandir` | `2.1.5` | MIT | No |
| `@nodelib/fs.stat` | `2.0.5` | MIT | No |
| `@nodelib/fs.walk` | `1.2.8` | MIT | No |
| `@nolyfill/is-core-module` | `1.0.39` | MIT | No |
| `@peculiar/asn1-schema` | `2.8.0` | MIT | No |
| `@peculiar/json-schema` | `1.1.12` | MIT | No |
| `@peculiar/utils` | `2.0.3` | MIT | No |
| `@peculiar/webcrypto` | `1.7.1` | MIT | No |
| `@rtsao/scc` | `1.1.0` | MIT | No |
| `@sindresorhus/is` | `4.6.0` | MIT | No |
| `@szmarczak/http-timer` | `4.0.6` | MIT | No |
| `@tailwindcss/node` | `4.3.3` | MIT | No |
| `@tailwindcss/oxide` | `4.3.3` | MIT | No |
| `@tailwindcss/oxide-android-arm64` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-darwin-arm64` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-darwin-x64` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-freebsd-x64` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-linux-arm-gnueabihf` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-linux-arm64-gnu` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-linux-arm64-musl` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-linux-x64-gnu` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-linux-x64-musl` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-wasm32-wasi` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-win32-arm64-msvc` | `4.3.3` | MIT | Yes |
| `@tailwindcss/oxide-win32-x64-msvc` | `4.3.3` | MIT | Yes |
| `@tailwindcss/postcss` | `4.3.3` | MIT | No |
| `@tweenjs/tween.js` | `23.1.3` | MIT | No |
| `@tybys/wasm-util` | `0.10.1` | MIT | Yes |
| `@tybys/wasm-util` | `0.10.2` | MIT | Yes |
| `@types/cacheable-request` | `6.0.3` | MIT | No |
| `@types/fs-extra` | `9.0.13` | MIT | No |
| `@types/http-cache-semantics` | `4.2.0` | MIT | No |
| `@types/json-schema` | `7.0.15` | MIT | No |
| `@types/json5` | `0.0.29` | MIT | No |
| `@types/keyv` | `3.1.4` | MIT | No |
| `@types/node` | `24.13.3` | MIT | No |
| `@types/node` | `25.0.10` | MIT | No |
| `@types/proper-lockfile` | `4.1.4` | MIT | No |
| `@types/react` | `19.2.18` | MIT | No |
| `@types/react-dom` | `19.2.4` | MIT | No |
| `@types/react-syntax-highlighter` | `15.5.13` | MIT | No |
| `@types/responselike` | `1.0.3` | MIT | No |
| `@types/stats.js` | `0.17.4` | MIT | No |
| `@types/three` | `0.185.4` | MIT | No |
| `@types/webxr` | `0.5.24` | MIT | No |
| `@typescript-eslint/eslint-plugin` | `8.57.1` | MIT | No |
| `@typescript-eslint/parser` | `8.57.1` | MIT | No |
| `@typescript-eslint/project-service` | `8.57.1` | MIT | No |
| `@typescript-eslint/scope-manager` | `8.57.1` | MIT | No |
| `@typescript-eslint/tsconfig-utils` | `8.57.1` | MIT | No |
| `@typescript-eslint/type-utils` | `8.57.1` | MIT | No |
| `@typescript-eslint/types` | `8.57.1` | MIT | No |
| `@typescript-eslint/typescript-estree` | `8.57.1` | MIT | No |
| `@typescript-eslint/utils` | `8.57.1` | MIT | No |
| `@typescript-eslint/visitor-keys` | `8.57.1` | MIT | No |
| `@unrs/resolver-binding-android-arm-eabi` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-android-arm64` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-darwin-arm64` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-darwin-x64` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-freebsd-x64` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-arm-gnueabihf` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-arm-musleabihf` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-arm64-gnu` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-arm64-musl` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-ppc64-gnu` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-riscv64-gnu` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-riscv64-musl` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-s390x-gnu` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-x64-gnu` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-linux-x64-musl` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-wasm32-wasi` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-win32-arm64-msvc` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-win32-ia32-msvc` | `1.11.1` | MIT | Yes |
| `@unrs/resolver-binding-win32-x64-msvc` | `1.11.1` | MIT | Yes |
| `abbrev` | `4.0.0` | ISC | No |
| `acorn-jsx` | `5.3.2` | MIT | No |
| `ajv` | `6.14.0` | MIT | No |
| `ajv` | `6.15.0` | MIT | No |
| `app-builder-lib` | `26.15.3` | MIT | No |
| `aria-query` | `5.3.2` | Apache-2.0 | No |
| `array-buffer-byte-length` | `1.0.2` | MIT | No |
| `array-includes` | `3.1.9` | MIT | No |
| `array.prototype.findlast` | `1.2.5` | MIT | No |
| `array.prototype.findlastindex` | `1.2.6` | MIT | No |
| `array.prototype.flat` | `1.3.3` | MIT | No |
| `array.prototype.flatmap` | `1.3.3` | MIT | No |
| `array.prototype.tosorted` | `1.1.4` | MIT | No |
| `arraybuffer.prototype.slice` | `1.0.4` | MIT | No |
| `asn1js` | `3.0.10` | BSD-3-Clause | No |
| `ast-types-flow` | `0.0.8` | MIT | No |
| `async-exit-hook` | `2.0.1` | MIT | No |
| `async-function` | `1.0.0` | MIT | No |
| `at-least-node` | `1.0.0` | ISC | No |
| `available-typed-arrays` | `1.0.7` | MIT | No |
| `aws4` | `1.13.2` | MIT | No |
| `axe-core` | `4.11.1` | MPL-2.0 | No |
| `axobject-query` | `4.1.0` | Apache-2.0 | No |
| `boolean` | `3.2.0` | MIT | Yes |
| `brace-expansion` | `1.1.18` | MIT | No |
| `braces` | `3.0.3` | MIT | No |
| `browserslist` | `4.28.7` | MIT | No |
| `builder-util` | `26.15.3` | MIT | No |
| `bytestreamjs` | `2.0.1` | BSD-3-Clause | No |
| `cacheable-lookup` | `5.0.4` | MIT | No |
| `cacheable-request` | `7.0.4` | MIT | No |
| `call-bind` | `1.0.8` | MIT | No |
| `chalk` | `4.1.2` | MIT | No |
| `chromium-pickle-js` | `0.2.0` | MIT | No |
| `ci-info` | `4.3.1` | MIT | No |
| `ci-info` | `4.4.0` | MIT | No |
| `clone-response` | `1.0.3` | MIT | No |
| `commander` | `5.1.0` | MIT | No |
| `compare-version` | `0.1.2` | MIT | No |
| `concat-map` | `0.0.1` | MIT | No |
| `convert-source-map` | `2.0.0` | MIT | No |
| `damerau-levenshtein` | `1.0.8` | BSD-2-Clause | No |
| `data-view-buffer` | `1.0.2` | MIT | No |
| `data-view-byte-length` | `1.0.2` | MIT | No |
| `data-view-byte-offset` | `1.0.1` | MIT | No |
| `debug` | `3.2.7` | MIT | No |
| `decompress-response` | `6.0.0` | MIT | No |
| `deep-is` | `0.1.4` | MIT | No |
| `defer-to-connect` | `2.0.1` | MIT | No |
| `define-data-property` | `1.1.4` | MIT | No |
| `define-properties` | `1.2.1` | MIT | No |
| `detect-node` | `2.1.0` | MIT | Yes |
| `dir-compare` | `4.2.0` | MIT | No |
| `dmg-builder` | `26.15.3` | MIT | No |
| `doctrine` | `2.1.0` | Apache-2.0 | No |
| `dotenv` | `16.6.1` | BSD-2-Clause | No |
| `dotenv-expand` | `11.0.7` | BSD-2-Clause | No |
| `duplexer2` | `0.1.4` | BSD-3-Clause | No |
| `ejs` | `3.1.10` | Apache-2.0 | No |
| `electron` | `43.4.0` | MIT | No |
| `electron-builder` | `26.15.3` | MIT | No |
| `electron-publish` | `26.15.3` | MIT | No |
| `electron-to-chromium` | `1.5.399` | ISC | No |
| `end-of-stream` | `1.4.5` | MIT | No |
| `enhanced-resolve` | `5.24.5` | MIT | No |
| `env-paths` | `2.2.1` | MIT | No |
| `env-paths` | `3.0.0` | MIT | No |
| `err-code` | `2.0.3` | MIT | No |
| `es-abstract` | `1.24.1` | MIT | No |
| `es-iterator-helpers` | `1.3.1` | MIT | No |
| `es-shim-unscopables` | `1.1.0` | MIT | No |
| `es-to-primitive` | `1.3.0` | MIT | No |
| `es6-error` | `4.1.1` | MIT | Yes |
| `eslint` | `9.39.5` | MIT | No |
| `eslint-config-next` | `16.3.3` | MIT | No |
| `eslint-import-resolver-node` | `0.3.9` | MIT | No |
| `eslint-import-resolver-typescript` | `3.10.1` | ISC | No |
| `eslint-module-utils` | `2.12.1` | MIT | No |
| `eslint-plugin-import` | `2.32.0` | MIT | No |
| `eslint-plugin-jsx-a11y` | `6.10.2` | MIT | No |
| `eslint-plugin-react` | `7.37.5` | MIT | No |
| `eslint-plugin-react-hooks` | `7.1.1` | MIT | No |
| `eslint-scope` | `8.4.0` | BSD-2-Clause | No |
| `eslint-visitor-keys` | `3.4.3` | Apache-2.0 | No |
| `eslint-visitor-keys` | `4.2.1` | Apache-2.0 | No |
| `eslint-visitor-keys` | `5.0.1` | Apache-2.0 | No |
| `espree` | `10.4.0` | BSD-2-Clause | No |
| `esquery` | `1.7.0` | BSD-3-Clause | No |
| `esrecurse` | `4.3.0` | BSD-2-Clause | No |
| `estraverse` | `5.3.0` | BSD-2-Clause | No |
| `esutils` | `2.0.3` | BSD-2-Clause | No |
| `exponential-backoff` | `3.1.3` | Apache-2.0 | No |
| `fast-glob` | `3.3.1` | MIT | No |
| `fast-json-stable-stringify` | `2.1.0` | MIT | No |
| `fast-levenshtein` | `2.0.6` | MIT | No |
| `fastq` | `1.20.1` | ISC | No |
| `fdir` | `6.5.0` | MIT | No |
| `fflate` | `0.8.3` | MIT | No |
| `file-entry-cache` | `8.0.0` | MIT | No |
| `filelist` | `1.0.6` | Apache-2.0 | No |
| `fill-range` | `7.1.1` | MIT | No |
| `find-up` | `5.0.0` | MIT | No |
| `flat-cache` | `4.0.1` | MIT | No |
| `flatted` | `3.4.2` | ISC | No |
| `for-each` | `0.3.5` | MIT | No |
| `fs-extra` | `11.3.1` | MIT | No |
| `fs-extra` | `11.4.0` | MIT | No |
| `fs-extra` | `8.1.0` | MIT | No |
| `fs-extra` | `9.1.0` | MIT | No |
| `fs.realpath` | `1.0.0` | ISC | No |
| `function.prototype.name` | `1.1.8` | MIT | No |
| `functions-have-names` | `1.2.3` | MIT | No |
| `generator-function` | `2.0.1` | MIT | No |
| `gensync` | `1.0.0-beta.2` | MIT | No |
| `get-stream` | `5.2.0` | MIT | No |
| `get-symbol-description` | `1.1.0` | MIT | No |
| `get-tsconfig` | `4.13.7` | MIT | No |
| `glob` | `7.2.3` | ISC | No |
| `glob-parent` | `5.1.2` | ISC | No |
| `glob-parent` | `6.0.2` | ISC | No |
| `global-agent` | `3.0.0` | BSD-3-Clause | Yes |
| `globals` | `14.0.0` | MIT | No |
| `globals` | `16.4.0` | MIT | No |
| `globalthis` | `1.0.4` | MIT | No |
| `got` | `11.8.6` | MIT | No |
| `has-bigints` | `1.1.0` | MIT | No |
| `has-flag` | `4.0.0` | MIT | No |
| `has-property-descriptors` | `1.0.2` | MIT | No |
| `has-proto` | `1.2.0` | MIT | No |
| `hermes-estree` | `0.25.1` | MIT | No |
| `hermes-parser` | `0.25.1` | MIT | No |
| `hosted-git-info` | `4.1.0` | ISC | No |
| `http-cache-semantics` | `4.2.0` | BSD-2-Clause | No |
| `http2-wrapper` | `1.0.3` | MIT | No |
| `ignore` | `5.3.2` | MIT | No |
| `imurmurhash` | `0.1.4` | MIT | No |
| `inflight` | `1.0.6` | ISC | No |
| `internal-slot` | `1.1.0` | MIT | No |
| `is-array-buffer` | `3.0.5` | MIT | No |
| `is-async-function` | `2.1.1` | MIT | No |
| `is-bigint` | `1.1.0` | MIT | No |
| `is-boolean-object` | `1.2.2` | MIT | No |
| `is-bun-module` | `2.0.0` | MIT | No |
| `is-callable` | `1.2.7` | MIT | No |
| `is-data-view` | `1.0.2` | MIT | No |
| `is-date-object` | `1.1.0` | MIT | No |
| `is-extglob` | `2.1.1` | MIT | No |
| `is-finalizationregistry` | `1.1.1` | MIT | No |
| `is-generator-function` | `1.1.2` | MIT | No |
| `is-glob` | `4.0.3` | MIT | No |
| `is-map` | `2.0.3` | MIT | No |
| `is-negative-zero` | `2.0.3` | MIT | No |
| `is-number` | `7.0.0` | MIT | No |
| `is-number-object` | `1.1.1` | MIT | No |
| `is-regex` | `1.2.1` | MIT | No |
| `is-set` | `2.0.3` | MIT | No |
| `is-shared-array-buffer` | `1.0.4` | MIT | No |
| `is-string` | `1.1.1` | MIT | No |
| `is-symbol` | `1.1.1` | MIT | No |
| `is-typed-array` | `1.1.15` | MIT | No |
| `is-weakmap` | `2.0.2` | MIT | No |
| `is-weakref` | `1.1.1` | MIT | No |
| `is-weakset` | `2.0.4` | MIT | No |
| `isarray` | `2.0.5` | MIT | No |
| `isbinaryfile` | `4.0.10` | MIT | No |
| `isbinaryfile` | `5.0.7` | MIT | No |
| `isexe` | `3.1.5` | BlueOak-1.0.0 | No |
| `isexe` | `4.0.0` | BlueOak-1.0.0 | No |
| `iterator.prototype` | `1.1.5` | MIT | No |
| `jake` | `10.9.4` | Apache-2.0 | No |
| `json-buffer` | `3.0.1` | MIT | No |
| `json-schema-traverse` | `0.4.1` | MIT | No |
| `json-stable-stringify-without-jsonify` | `1.0.1` | MIT | No |
| `json-stringify-safe` | `5.0.1` | ISC | Yes |
| `json5` | `1.0.2` | MIT | No |
| `jsonfile` | `4.0.0` | MIT | No |
| `jsx-ast-utils` | `3.3.5` | MIT | No |
| `keyv` | `4.5.4` | MIT | No |
| `language-subtag-registry` | `0.3.23` | CC0-1.0 | No |
| `language-tags` | `1.0.9` | MIT | No |
| `levn` | `0.4.1` | MIT | No |
| `lightningcss` | `1.32.0` | MPL-2.0 | No |
| `lightningcss-android-arm64` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-darwin-arm64` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-darwin-x64` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-freebsd-x64` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-linux-arm-gnueabihf` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-linux-arm64-gnu` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-linux-arm64-musl` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-linux-x64-gnu` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-linux-x64-musl` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-win32-arm64-msvc` | `1.32.0` | MPL-2.0 | Yes |
| `lightningcss-win32-x64-msvc` | `1.32.0` | MPL-2.0 | Yes |
| `locate-path` | `6.0.0` | MIT | No |
| `lodash.merge` | `4.6.2` | MIT | No |
| `loose-envify` | `1.4.0` | MIT | No |
| `lowercase-keys` | `2.0.0` | MIT | No |
| `lru-cache` | `5.1.1` | ISC | No |
| `lru-cache` | `6.0.0` | ISC | No |
| `magic-string` | `0.30.21` | MIT | No |
| `matcher` | `3.0.0` | MIT | Yes |
| `merge2` | `1.4.1` | MIT | No |
| `meshoptimizer` | `1.1.1` | MIT | No |
| `micromatch` | `4.0.8` | MIT | No |
| `mime` | `2.6.0` | MIT | No |
| `mimic-response` | `1.0.1` | MIT | No |
| `mimic-response` | `3.1.0` | MIT | No |
| `minimatch` | `10.2.6` | BlueOak-1.0.0 | No |
| `minimatch` | `3.1.5` | ISC | No |
| `minimist` | `1.2.8` | MIT | No |
| `napi-postinstall` | `0.3.4` | MIT | No |
| `natural-compare` | `1.4.0` | MIT | No |
| `node-abi` | `4.33.0` | MIT | No |
| `node-api-version` | `0.2.1` | MIT | No |
| `node-exports-info` | `1.6.0` | MIT | No |
| `node-gyp` | `12.4.0` | MIT | No |
| `node-int64` | `0.4.0` | MIT | No |
| `node-releases` | `2.0.51` | MIT | No |
| `nopt` | `9.0.0` | ISC | No |
| `normalize-url` | `6.1.0` | MIT | No |
| `object-keys` | `1.1.1` | MIT | No |
| `object.assign` | `4.1.7` | MIT | No |
| `object.entries` | `1.1.9` | MIT | No |
| `object.fromentries` | `2.0.8` | MIT | No |
| `object.groupby` | `1.0.3` | MIT | No |
| `object.values` | `1.2.1` | MIT | No |
| `optionator` | `0.9.4` | MIT | No |
| `own-keys` | `1.0.1` | MIT | No |
| `p-cancelable` | `2.1.1` | MIT | No |
| `p-limit` | `3.1.0` | MIT | No |
| `p-locate` | `5.0.0` | MIT | No |
| `path-exists` | `4.0.0` | MIT | No |
| `pe-library` | `0.4.1` | MIT | No |
| `picomatch` | `2.3.2` | MIT | No |
| `picomatch` | `4.0.4` | MIT | No |
| `pkijs` | `3.4.0` | BSD-3-Clause | No |
| `plist` | `3.1.0` | MIT | No |
| `possible-typed-array-names` | `1.1.0` | MIT | No |
| `prelude-ls` | `1.2.1` | MIT | No |
| `proc-log` | `6.1.0` | ISC | No |
| `progress` | `2.0.3` | MIT | No |
| `promise-retry` | `2.0.1` | MIT | No |
| `prop-types` | `15.8.1` | MIT | No |
| `pump` | `3.0.4` | MIT | No |
| `punycode` | `2.3.1` | MIT | No |
| `pvtsutils` | `1.3.6` | MIT | No |
| `pvutils` | `1.1.5` | MIT | No |
| `queue-microtask` | `1.2.3` | MIT | No |
| `quick-lru` | `5.1.1` | MIT | No |
| `read-binary-file-arch` | `1.0.6` | MIT | No |
| `reflect.getprototypeof` | `1.0.10` | MIT | No |
| `regexp.prototype.flags` | `1.5.4` | MIT | No |
| `resedit` | `1.7.2` | MIT | No |
| `resolve` | `2.0.0-next.6` | MIT | No |
| `resolve-alpn` | `1.2.1` | MIT | No |
| `resolve-pkg-maps` | `1.0.0` | MIT | No |
| `responselike` | `2.0.1` | MIT | No |
| `reusify` | `1.1.0` | MIT | No |
| `roarr` | `2.15.4` | BSD-3-Clause | Yes |
| `run-parallel` | `1.2.0` | MIT | No |
| `safe-array-concat` | `1.1.3` | MIT | No |
| `safe-push-apply` | `1.0.0` | MIT | No |
| `safe-regex-test` | `1.1.0` | MIT | No |
| `sanitize-filename` | `1.6.4` | WTFPL OR ISC | No |
| `semver` | `5.7.2` | ISC | No |
| `semver` | `6.3.1` | ISC | No |
| `semver-compare` | `1.0.0` | MIT | Yes |
| `serialize-error` | `7.0.1` | MIT | Yes |
| `set-function-length` | `1.2.2` | MIT | No |
| `set-function-name` | `2.0.2` | MIT | No |
| `set-proto` | `1.0.0` | MIT | No |
| `simple-update-notifier` | `2.0.0` | MIT | No |
| `sprintf-js` | `1.1.3` | BSD-3-Clause | Yes |
| `stable-hash` | `0.0.5` | MIT | No |
| `stat-mode` | `1.0.0` | MIT | No |
| `stop-iteration-iterator` | `1.1.0` | MIT | No |
| `string.prototype.includes` | `2.0.1` | MIT | No |
| `string.prototype.matchall` | `4.0.12` | MIT | No |
| `string.prototype.repeat` | `1.0.0` | MIT | No |
| `string.prototype.trim` | `1.2.10` | MIT | No |
| `string.prototype.trimend` | `1.0.9` | MIT | No |
| `string.prototype.trimstart` | `1.0.8` | MIT | No |
| `strip-bom` | `3.0.0` | MIT | No |
| `strip-json-comments` | `3.1.1` | MIT | No |
| `sumchecker` | `3.0.1` | Apache-2.0 | No |
| `supports-color` | `7.2.0` | MIT | No |
| `tailwindcss` | `4.3.3` | MIT | No |
| `tapable` | `2.3.3` | MIT | No |
| `temp-file` | `3.4.0` | MIT | No |
| `tiny-async-pool` | `1.3.0` | MIT | No |
| `tinyglobby` | `0.2.15` | MIT | No |
| `tmp` | `0.2.7` | MIT | No |
| `tmp-promise` | `3.0.3` | MIT | No |
| `to-regex-range` | `5.0.1` | MIT | No |
| `truncate-utf8-bytes` | `1.0.2` | WTFPL | No |
| `ts-api-utils` | `2.5.0` | MIT | No |
| `tsconfig-paths` | `3.15.0` | MIT | No |
| `type-check` | `0.4.0` | MIT | No |
| `type-fest` | `0.13.1` | (MIT OR CC0-1.0) | Yes |
| `typed-array-buffer` | `1.0.3` | MIT | No |
| `typed-array-byte-length` | `1.0.3` | MIT | No |
| `typed-array-byte-offset` | `1.0.4` | MIT | No |
| `typed-array-length` | `1.0.7` | MIT | No |
| `typescript` | `5.9.3` | Apache-2.0 | No |
| `typescript-eslint` | `8.57.1` | MIT | No |
| `unbox-primitive` | `1.1.0` | MIT | No |
| `undici` | `6.28.0` | MIT | No |
| `undici` | `7.29.0` | MIT | Yes |
| `undici-types` | `7.16.0` | MIT | No |
| `undici-types` | `7.18.2` | MIT | No |
| `universalify` | `0.1.2` | MIT | No |
| `unrs-resolver` | `1.11.1` | MIT | No |
| `unzipper` | `0.12.5` | MIT | No |
| `update-browserslist-db` | `1.2.3` | MIT | No |
| `uri-js` | `4.4.1` | BSD-2-Clause | No |
| `utf8-byte-length` | `1.0.5` | (WTFPL OR MIT) | No |
| `webcrypto-core` | `1.9.2` | MIT | No |
| `which` | `5.0.0` | ISC | No |
| `which` | `6.0.1` | ISC | No |
| `which-boxed-primitive` | `1.1.1` | MIT | No |
| `which-builtin-type` | `1.2.1` | MIT | No |
| `which-collection` | `1.0.2` | MIT | No |
| `which-typed-array` | `1.1.20` | MIT | No |
| `word-wrap` | `1.2.5` | MIT | No |
| `xmlbuilder` | `15.1.1` | MIT | No |
| `yallist` | `3.1.1` | ISC | No |
| `yallist` | `4.0.0` | ISC | No |
| `yocto-queue` | `0.1.0` | MIT | No |
| `zod-validation-error` | `4.0.2` | MIT | No |
