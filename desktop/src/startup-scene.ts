...
export const STARTUP_CONTINUE_URL = "piora-startup://continue";
...
<meta charset="utf-8"><meta name="color-scheme" content="dark"><meta name="theme-color" content="#080a0f">
...
<header class="top"><div class="wordmark">π / PIORA</div><div class="mission"><span class="live"></span>POLARIS EXPEDITION<br>ROVER 01 · 89° 15′ N</div></header>
<main><div><div class="eyebrow">${updated ? "A NEW HORIZON AWAITS" : "BEYOND THE NEXT HORIZON"}</div><h1 class="title">PIORA</h1><div class="subtitle">${zh ? updated ? "更新就绪。下一程，驶向未知。" : "保持好奇，驶向未知。" : updated ? "Update ready. A new frontier awaits." : "Stay curious. Explore what comes next."}</div></div><div class="statusbox"><div class="status" role="status">${zh ? "正在启动 Piora" : "Starting Piora"}<br>${zh ? "正在准备你的工作区" : "Preparing your workspace"}</div><div class="progress" role="progressbar" aria-label="${zh ? "正在启动" : "Starting"}"></div><div class="actions"><span class="version">v${escapeHtml(options.version)}</span><a href="${STARTUP_CONTINUE_URL}">${zh ? "跳过动画" : "Skip intro"} ↗</a></div></div></main>
<script nonce="piora-startup">const clip=document.querySelector('video');const motion=matchMedia('(prefers-reduced-motion: reduce)');function settleMotion(){if(motion.matches){clip?.pause();location.href='${STARTUP_CONTINUE_URL}';}}motion.addEventListener('change',settleMotion);clip?.addEventListener('error',()=>{location.href='${STARTUP_CONTINUE_URL}';},true);settleMotion();</script>
</body></html>`;
}
