...
<script nonce="piora-startup">const clip=document.querySelector('video');const motion=matchMedia('(prefers-reduced-motion: reduce)');function continueStartup(){location.href='${STARTUP_CONTINUE_URL}';}function settleMotion(){if(motion.matches){clip?.pause();continueStartup();}}motion.addEventListener('change',settleMotion);clip?.addEventListener('error',continueStartup,true);clip?.addEventListener('ended',continueStartup,{once:true});settleMotion();</script>
</body></html>`;
}
