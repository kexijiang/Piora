...
test("silently skips absent providers but keeps the models that matched", async () => {
...
test("deleting a model removes it without warnings and preserves remaining thinking pins", async () => {
  const patterns = ["anthropic/claude-opus-5", "acme-gateway/claude-opus-4-8:high"];
  let available = MODELS;
  const changingRuntime = { getAvailable: async () => available };
  assert.equal((await resolveVisibleModels(changingRuntime, patterns)).visible.length, 2);
  available = MODELS.filter((model) => model.provider !== "anthropic");
  const result = await resolveVisibleModels(changingRuntime, patterns);
  assert.deepEqual(refs(result), ["acme-gateway/claude-opus-4-8"]);
  assert.deepEqual(result.warnings, []);
  assert.deepEqual(result.thinkingLevelPins, { "acme-gateway/claude-opus-4-8": "high" });
});

test("invalid thinking settings still warn for present models, but not deleted ones", async () => {
  const result = await resolveVisibleModels(runtime, ["anthropic/claude-opus-5:invalid", "deleted/model:invalid"]);
  assert.equal(result.warnings.length, 1);
  assert.match(result.warnings[0], /invalid/);
  assert.doesNotMatch(result.warnings[0], /deleted/);
  assert.deepEqual(refs(result), ["anthropic/claude-opus-5"]);
});

test("an unavailable startup model only falls back when explicitly enabled", async () => {
  const scope = await resolveVisibleModels(runtime, ["anthropic/*:high"]);
  const requestedModel = { provider: "offline", modelId: "missing" };
  assert.throws(() => selectInitialModelScope(scope, { requestedModel }), /not available/);
  const initial = selectInitialModelScope(scope, { requestedModel, allowUnavailableRequestedModel: true });
  assert.equal(initial.model.provider, "anthropic");
  assert.equal(initial.thinkingLevel, "high");
  const invalidScope = await resolveVisibleModels(runtime, ["missing/*"]);
  assert.throws(() => selectInitialModelScope(invalidScope, { requestedModel, allowUnavailableRequestedModel: true }), /not available/);
});
