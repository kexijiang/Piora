...
test("a partial catalog error is not cached after the underlying config is repaired", async () => {
  invalidateModelsCache();
  const broken = await loadModelsWithCache("/repaired", async () => withModelRuntimeError(modelsData("builtin"), "missing cacheWrite"));
  assert.equal(broken.modelError, "missing cacheWrite");
  const repaired = await loadModelsWithCache("/repaired", async () => modelsData("custom"));
  assert.equal(repaired.modelError, undefined);
  assert.equal(repaired.modelList[0].id, "custom");
});
