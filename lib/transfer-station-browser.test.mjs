...
window.records=initial;window.writes=[];function App(){const [items,setItems]=React.useState(initial);return <main className="content" data-tool="library" style={{padding:24}}><div className="libraryPane"><CompanionTransferStation items={items} loaded loading={false} pending={false} error="" refresh={async()=>{}} write={async(method,input)=>{window.writes.push(input);if(window.failSave&&input.content!==undefined)throw new Error("保存失败测试");if(window.failDelete&&input.remove)throw new Error("删除失败测试");window.records=input.remove?window.records.filter(x=>x.id!==input.id):window.records.map(x=>x.id===input.id?{...x,...input,updatedAt:x.updatedAt+1}:x);setItems(window.records);return window.records;}}/></div></main>};createRoot(document.getElementById("root")).render(<App/>);`);
    ...
    const editorCss = (await Promise.all(["node_modules/vditor/dist/index.css", "components/MarkdownEditor.css"].map(file => readFile(path.join(repo,file),"utf8")))).join("\n");
    ...
    await page.route("http://transfer-ui.test/**", async (route) => {
      ...
      if (url.pathname.startsWith("/vendor/vditor/4.0.0/dist/")) await route.fulfill({ contentType: "text/javascript", body: await readFile(path.join(repo,"node_modules/vditor/dist",url.pathname.slice("/vendor/vditor/4.0.0/dist/".length))) });
    ...
    await page.locator('[data-mode="sv"]').evaluate(button=>button.click());
    ...
    assert.equal(await page.getByLabel("Markdown 源码", { exact: true }).inputValue(), "不能丢的草稿");
