...
test("folds the title request into a trailing user message without mutating the source", () => {
  ...
  assert.match(prepared[1].content.at(-1).text, /precise session title editor/i);
...
test("temporary title agent preserves the provider-facing prefix", async () => {
  ...
  const titleModel = { provider: "fast-provider", id: "fast-title-model" };
  const overridden = buildSessionTitleAgentOptions(source, { model: titleModel });
  assert.equal(overridden.initialState.model, titleModel);
  assert.equal(overridden.initialState.thinkingLevel, "off");
});

test("cancels while waiting for a busy source session", async () => {
  ...
  const sourceAgent = {
    state: {
      ...
      thinkingLevel: "off",
      tools: [],
      messages: [{ role: "user", content: "Rename this", timestamp: 1 }],
    },
    waitForIdle: () => new Promise(() => {}),
  };

  const pending = generateSessionTitle({ agent: sourceAgent }, { signal: controller.signal });
  controller.abort();
...
test("removes incomplete tool calls before invoking the title provider", async () => {
  ...
  assert.match(providerMessages[0].content, /precise session title editor/i);
});
