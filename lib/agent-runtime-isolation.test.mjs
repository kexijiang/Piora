...
test("ordinary sessions expose every first-party extension to the load plan", () => {
  assert.deepEqual(
    ...
    ["piora-automations.ts", "piora-browser.ts", "piora-computer.ts", "piora-file-changes.ts", "piora-goal.ts", "piora-harmony.ts", "piora-plan.ts", "piora-room.ts", "piora-shell-guard.ts", "piora-ssh.ts", "piora-vision-agent.ts"],
  );
});
...
test("prompt lifecycle cleanup is tied to final prompt settlement, abort, fork, and destroy", () => {
  ...
  assert.match(rpc, /writers\.push\(finishPromptRun\(promptRun, "destroy"\)\)/);
  assert.match(rpc, /await Promise\.allSettled\(writers\)/);
