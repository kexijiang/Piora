...
test("packaged Electron PTY reads and writes through the real native sidecar", { skip: process.platform !== "win32", timeout: 120_000 }, async (t) => {
  ...
  await writeFile(join(source, "probe.cjs"), `
    ...
    const child = pty.spawn('cmd.exe', ['/D', '/Q', '/K'], {cwd: process.cwd(), cols: 90, rows: 24, useConptyDll: true});
    child.onData(data => process.stdout.write(data));
    child.onExit(({exitCode}) => process.exit(exitCode));
    child.write('set PIORA_PTY_VALUE=OK\\r');
    child.write('echo PIORA_PACKAGED_PTY_%PIORA_PTY_VALUE%\\r');
    child.write('exit\\r');
  ...
  assert.ok(resolve(source).startsWith(`${resolve(root)}\\`));
