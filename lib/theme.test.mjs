...
test("keeps appearance discoverable inside the consolidated settings page, but not duplicate menus", () => {
  ...
  const navigationSource = readFileSync(resolve(projectRoot, "lib/settings-search.ts"), "utf8");
  ...
  assert.match(navigationSource, /key:\s*"appearance"/);
  assert.match(settingsSource, /onActiveKeyChange\(entry\.key\)/);
  assert.match(source, /appearance:\s*<AppearanceSettings \/>/);
  assert.match(readFileSync(resolve(projectRoot, "components/AppearanceThemeSettings.tsx"), "utf8"), /<AppearanceLooks \/>/);
  ...
  assert.match(readFileSync(resolve(projectRoot, "components/AppearanceThemeSettings.tsx"), "utf8"), /data-theme-id=\{preset\.id\}/);
});
...
test("offers one action that restores every appearance preference", () => {
  ...
  assert.match(shell, /<AppearanceSettings \/>/);
  assert.match(readFileSync(resolve(projectRoot, "components/AppearanceSettings.tsx"), "utf8"), /<AppearanceResetButton compact \/>/);
