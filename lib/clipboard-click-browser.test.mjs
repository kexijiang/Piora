...
for (const surface of ["pocket", "quick"]) test(`${surface} clipboard copies on click with a small transient acknowledgement`, { skip: process.env.PIORA_SKIP_RESOURCE_TESTS === "1", timeout: 120000 }, async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "piora-clipboard-click-"));
  let browser;
  try {
    const { bundle, css } = await buildClipboardUI(directory, { workbench: surface === "pocket" });
    browser = await chromium.launch({ channel: "msedge", headless: true });
    const page = await browser.newPage({ viewport: surface === "quick" ? { width: 760, height: 540 } : { width: 1000, height: 850 } });
    const errors = [];
    ...
    await page.goto(`https://clipboard-click.test/?surface=${surface === "quick" ? "quick" : "manager"}`);
    ...
    await toast.waitFor({ state: "hidden", timeout: 3000 });
    const copiesBeforeEnter = await page.evaluate(() => window.commands.filter(command => command.type === "copy").length);
    await page.getByLabel("搜索剪贴板").press("Enter");
    await page.waitForFunction(before => window.commands.filter(command => command.type === "copy").length > before, copiesBeforeEnter);
    assert.equal(await page.evaluate(() => window.commands.filter(command => command.type === "paste").length), 0, "Enter only copies; users paste themselves");
    ...
    await browser?.close();
    assert.ok(path.resolve(directory).startsWith(path.resolve(tmpdir()) + path.sep) && path.basename(directory).startsWith("piora-clipboard-click-"));
