...
export async function buildClipboardUI(directory, { draftTesting = false, workbench = false } = {}) {
  ...
  await writeFile(path.join(directory, "entry.tsx"), `${draftTesting ? `import * as draftStore from ${JSON.stringify(path.join(repo, "components/clipboard/clipboard-draft-store.ts"))};window.testDraftStore=draftStore;` : ""}import React from "react";import {createRoot} from "react-dom/client";import {I18nProvider} from ${JSON.stringify(path.join(repo, "hooks/useI18n.tsx"))};import {${workbench ? "ClipboardWorkbench" : "ClipboardWorkspace"}} from ${JSON.stringify(path.join(repo, workbench ? "components/ClipboardWorkbench.tsx" : "components/clipboard/ClipboardWorkspace.tsx"))};if(!localStorage.getItem("pi-locale"))localStorage.setItem("pi-locale","zh-CN");createRoot(document.getElementById("root")).render(<I18nProvider>${workbench ? "<ClipboardWorkbench onSave={async()=>{}}/>" : '<ClipboardWorkspace surface={new URLSearchParams(location.search).get("surface")||"manager"}/>'}</I18nProvider>);`);
