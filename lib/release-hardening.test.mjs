...
test("release workflows use immutable actions and gate the packaged runtime before publication", async () => {
  const [
    ...
    readFile(join(repositoryRoot, ".github", "workflows", "test-suite.yml"), "utf8"),
  ...
  for (const workflow of [releaseWorkflow, harmonyWorkflow, ciWorkflow, testWorkflow]) {
  ...
  const preview = yaml.load(harmonyWorkflow);
  const ci = yaml.load(ciWorkflow);
  const suite = yaml.load(testWorkflow);
  for (const workflow of [release, preview, ci]) {
    assert.equal(workflow.jobs.tests.uses, "./.github/workflows/test-suite.yml");
  }
  assert.deepEqual(JSON.parse(suite.on.workflow_call.inputs.platforms.default), ["ubuntu-latest", "windows-latest"]);
  assert.deepEqual(JSON.parse(preview.jobs.tests.with.platforms), ["windows-latest"]);
  assert.equal(release.jobs.tests.if, release.jobs["source-gate"].if);
  assert.deepEqual(suite.jobs.tests.strategy.matrix.shard, [1, 2, 3]);
  assert.equal(suite.jobs.tests.strategy.matrix.os, "${{ fromJSON(inputs.platforms) }}");
  assert.equal(suite.jobs.tests.strategy["fail-fast"], false);
  assert.equal(suite.jobs.tests.steps.find(step => step.name === "Build desktop test fixtures").run, "npm run build:desktop");
  const shardStep = suite.jobs.tests.steps.find(step => step.name === "Run test shard");
  assert.equal(shardStep.run, "node scripts/run-tests.mjs --test-shard=${{ matrix.shard }}/3");
  assert.equal(shardStep.env.PIORA_SKIP_RESOURCE_TESTS, "${{ runner.os == 'Windows' && '1' || '0' }}");
  const resourceSteps = suite.jobs.tests.steps.filter(step => step.run === "npm run test:shell-resource");
  assert.equal(resourceSteps.length, 1);
  assert.equal(resourceSteps[0].if, "runner.os == 'Windows' && matrix.shard == 1");
  assert.equal(resourceSteps[0].env?.PIORA_SKIP_RESOURCE_TESTS, undefined);
  for (const [workflow, jobName, dependencies] of [
    [release, "publish-release", ["tests", "windows-build", "linux-build"]],
    [preview, "publish-preview", ["tests", "windows-preview"]],
  ]) {
    const job = workflow.jobs[jobName];
    assert.deepEqual(job.needs, dependencies);
    assert.equal(job.if, undefined, "publication must require every dependency to succeed");
    assert.equal(job.permissions.contents, "write");
    assert.ok(job.steps.some(step => step.uses?.startsWith("actions/download-artifact@")));
    assert.ok(job.steps.every(step => !step.uses?.startsWith("actions/checkout@") && !/npm (ci|install|run)/.test(step.run ?? "")));
  }
  assert.equal(preview.permissions.contents, "read");
  assert.equal(preview.jobs["windows-preview"].needs, undefined, "preview packaging can run alongside tests");
  assert.equal(ci.jobs.quality.needs, "tests");
  assert.equal(ci.jobs.quality.if, "${{ !cancelled() }}");
  assert.equal(ci.jobs.quality.steps[0].if, "needs.tests.result != 'success'");
  assert.equal(ci.jobs.quality.steps[0].run, "exit 1", "failed shards must fail the existing required quality checks");
  ...
  for (const command of [
    ...
    assert.equal(release.jobs["source-gate"].steps.filter(step => step.run === command).length, 1,
      `${command} must run once in the shared source gate`);
    assert.ok(release.jobs["windows-build"].steps.every(step => step.run !== command),
      `${command} must not repeat after the source gate`);
  }
