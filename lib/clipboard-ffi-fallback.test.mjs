...
test('missing FFI binding leaves clipboard history and polling available', async () => {
  ...
  try {
    ...
    assert.equal((await runtime.store.query({ text: 'baseline' })).items.length, 1, 'the current system clipboard is available without manually enabling or capturing it');
