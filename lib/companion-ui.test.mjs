...
test("companion uses independent pet, bubble, and panel surfaces", async () => {
  const [desktop, preload, pet, petStyles, bubble, bubblePage, panel, panelStyles, todoList, jsonWorkbench, jsonCodeEditor, jsonWorkbenchStyles, layout, backgrounds] = await Promise.all([
    ...
    readFile(new URL("../components/CompanionTodoList.tsx", import.meta.url), "utf8"),
  ...
  assert.match(desktop, /guardCompanionWindow\(window, screen/);
  assert.doesNotMatch(desktop, /setIgnoreMouseEvents\([^\n]*forward: true/);
  ...
  assert.match(desktop, /positionCompanionBubble\(placed \?\?/);
  assert.match(desktop, /companionBubbleWindow\.setPosition\(x, y, false\)/);
  assert.match(desktop, /planCompanionMotion/);
  assert.match(desktop, /companionWindowGuard\?\.place\(point\)/);
  ...
  assert.match(panel, /<CompanionTransferStation/);
  assert.match(panel, /companion-panel-root/);
  assert.match(panel, /允许宠物自主随机移动/);
  assert.match(panel, /<CompanionTodoList/);
  ...
  assert.match(panel, /disabled=\{busy \|\| !question\.trim\(\)\}/);
  assert.match(todoList, /disabled=\{busy \|\| !draft\.trim\(\)/);
  assert.doesNotMatch(panel, /请填写标题/);
  assert.match(panel, /disabled=\{busy \|\| !memoryDraft\.trim\(\)\}/);
  ...
  assert.match(panel, /hidden=\{tab !== "json"\}/);
  ...
  assert.match(jsonWorkbench, /className=\{styles\.quickTools\}/);
  assert.doesNotMatch(jsonWorkbench, /<ActionMenu/);
  assert.match(jsonWorkbench, /temporaryContent/);
  ...
  assert.match(jsonCodeEditor, /lineNumbers\(\)/);
  assert.match(jsonCodeEditor, /foldGutter\(/);
  assert.match(jsonCodeEditor, /autocompletion\(\)/);
