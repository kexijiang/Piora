...
test("authorizes full output from a recorded manual or agent shell result", async () => {
  ...
  const toolResultEntry = {
    ...
    message: {
      ...
      details: { truncation: { truncated: true }, fullOutputPath: outputPath },
    },
  };

  assert.equal(isBashOutputPathReferencedByEntries(outputPath, [bashEntry]), true);
  assert.equal(isBashOutputPathReferencedByEntries(outputPath, [toolResultEntry]), true);
  assert.equal(isBashOutputPathReferencedByEntries(outputPath, [{ ...toolResultEntry, message: { ...toolResultEntry.message, toolName: "custom_tool" } }]), false);
