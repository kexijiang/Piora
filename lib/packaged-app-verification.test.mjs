...
test("packaged runtime verifies every first-party extension and workflow tool state", async () => {
  ...
  assert.match(verifier, /Optional extensions should be disabled by default/);
  assert.match(verifier, /verifyPackagedCoreTools\(initialTools\)/);
  assert.match(verifier, /Packaged startup media is missing or differs from source/);
