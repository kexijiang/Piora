...
test('edit recovery survives process restart, preserves independent windows, retries storage failures and exports orphan drafts', { timeout: 120000 }, async () => {
  ...
  try {
    ...
    const savedRemark = await page.getByLabel('剪贴板备注', { exact: true }).inputValue();
    await page.getByLabel('剪贴板备注', { exact: true }).fill('快速修改');
    await page.getByLabel('剪贴板备注', { exact: true }).fill(savedRemark);
    await page.waitForFunction(async () => (await window.testDraftStore.listClipboardDrafts()).total === 0);
    // IndexedDB deletion can finish before React renders the idle recovery
    // state. Wait for that user-visible acknowledgement before navigating.
    await page.getByText('正在保留编辑草稿…', { exact: true }).waitFor({ state: 'detached' });
    await page.getByText('编辑草稿已保留在本机', { exact: true }).waitFor({ state: 'detached' });
    await page.getByRole('button', { name: '保存备注', exact: true }).waitFor({ state: 'detached' });
