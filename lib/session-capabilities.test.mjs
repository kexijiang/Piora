...
test("initial device status contains only the active gateway, not retired disabled operations", () => {
  for (const profile of ["normal", "device-control"]) {
    const state = capabilities.createDefaultSessionCapabilitiesState(profile);
    const devices = state.items.filter((item) => item.kind === "device");
    assert.deepEqual(devices.map((item) => item.id), ["tool:harmony_control"]);
    assert.ok(devices.every((item) => item.available && item.enabled));
  }
});

test("legacy enabled Harmony operations migrate to the gateway while an explicit disabled gateway stays disabled", () => {
  const catalog = capabilities.buildSessionCapabilityCatalog(tools(["read", "harmony_control"]), "normal");
  const restore = (knownCapabilityIds) => capabilities.restoreSessionCapabilityPolicy([{
    type: "custom", customType: capabilities.SESSION_CAPABILITY_ENTRY_TYPE,
    data: { version: 1, revision: 1, preset: "custom", enabledCapabilityIds: ["tool:harmony_tap"], knownCapabilityIds, updatedAt: "2026-09-07T00:00:00.000Z" },
  }], catalog);
  assert.deepEqual(restore(["tool:harmony_tap"]).enabledCapabilityIds, ["tool:harmony_control"]);
  assert.deepEqual(restore(["tool:harmony_tap", "tool:harmony_control"]).enabledCapabilityIds, []);
  assert.ok(capabilities.createSessionCapabilityPolicy(undefined, catalog, "normal").enabledCapabilityIds.includes("tool:harmony_control"));
});
