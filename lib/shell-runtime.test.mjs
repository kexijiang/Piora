...
async function waitUntil(read, predicate, timeout = process.env.CI ? 60000 : 15000) {
...
test("managed PTY preserves state, certifies results, isolates sessions and deduplicates submissions", { timeout: 90000 }, async t => {
  ...
  for (const profile of profiles) {
    ...
    assert.equal(second.state.cwd, root); assert.notEqual(second.state.id, session.state.id);
