...
function fixture({ failedLoad = false, failedWrite = false, delayedLoad = false } = {}) {
  const window = new EventEmitter(); window.webContents = new EventEmitter();
  let settleLoad; let stopped = false;
  window.webContents.stop = () => { stopped = true; };
  Object.assign(window, { isDestroyed: () => false, show() {}, maximize() {}, loadFile: async (path) => { loaded.push(path); if (failedLoad) throw new Error("load failed"); if (delayedLoad) await new Promise(resolve => { settleLoad = resolve; }); } });
  ...
  return { ...start({ warn() {} }), loaded, writes, timers, settleLoad: () => settleLoad(), stopped: () => stopped };
}
...
test("watchdog handoff stops and settles a pending navigation before releasing the app page", async () => {
  ...
  f.timers[0].run(); await Promise.resolve();
