...
function fixture({ failedLoad = false, failedWrite = false, delayedLoad = false, abortOnStop = false, failedStop = false } = {}) {
  ...
  window.isDestroyed = webContents.isDestroyed = () => destroyed;
  ...
  return { ...start({ warn() {}, info() {} }), webContents, loaded, writes, timers, settleLoad: () => settleLoad(), stopped: () => stopped };
}

test("closing the main window after intro handoff does not access its destroyed native object", async () => {
  const f = fixture();
  f.timers[0].run(); await f.finished;
  f.webContents.emit("did-navigate", {}, "http://127.0.0.1:30141/");
  assert.doesNotThrow(() => f.window.destroy());
});

test("closing during the intro or its completion callback safely releases the handoff", async () => {
  for (const completing of [false, true]) {
    const f = fixture();
    if (completing) f.webContents.emit("ipc-message", { senderFrame: f.webContents.mainFrame }, "pi:startup-continue");
    assert.doesNotThrow(() => f.window.destroy());
    await f.finished;
    assert.equal(f.stopped(), false, "closed renderers must not receive native stop calls");
    for (const event of ["will-navigate", "did-navigate", "ipc-message"]) assert.equal(f.webContents.listenerCount(event), 0);
  }
});
