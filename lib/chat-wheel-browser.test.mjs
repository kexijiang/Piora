...
createRoot(document.getElementById("root")).render(location.search.includes("strict")?<React.StrictMode><App/></React.StrictMode>:<App/>);`);
    ...
    for (const {scale, strict} of [{scale:1}, {scale:1.25}, {scale:1.5}, {scale:1, strict:true}]) {
      ...
      // Allow a final layout/measurement frame to settle, then require the
      // trailing window to remain stable.  A single late frame is expected
      // when deferred rows finish measuring on hosted runners.
      const settled = samples.slice(-20);
      assert.equal(new Set(settled.map(sample => JSON.stringify(sample))).size, 1,
        `scale ${scale}: idle history keeps moving or remounting: ${JSON.stringify(settled)}`);
