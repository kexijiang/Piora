...
test("design components keep completion separate from execution and settings survive a failed save", { timeout: 180000 }, async () => {
  ...
  try {
    ...
    await writeFile(path.join(directory, "entry.tsx"), `import {createRoot} from "react-dom/client";
      ...
      function Transport(){const shell=useSmartShell('X:/workspace/sample');window.shell=shell;return <div><p>{shell.activeId}</p><p>{shell.snapshot?.session.id}</p><p>{shell.sessions.map(s=>s.id).join(',')}</p></div>}
    ...
    await page.route("https://shell-ui.test/**", async route => {
      ...
      if (url.pathname === "/api/shell/sessions") return route.fulfill({ json: { sessions: terminalList } });
    ...
    await page.waitForFunction(() => window.shell?.snapshot?.session.id === "first");
