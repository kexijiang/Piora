...
test("loads the built-in browser with a visible panel and persistent Piora profile", () => {
  ...
  assert.match(extension, /pages: Set<Page>/);
  assert.match(extension, /function sessionPages\(/);
  assert.match(extension, /trackSessionPage\(session, page\)/);
  assert.doesNotMatch(extension, /existing\.context\.close\(/);
  assert.doesNotMatch(extension, /session\.context\.pages\(\)/);
  assert.doesNotMatch(extension, /await persistBrowserState\(session\.context\)/);
  assert.match(extension, /PIORA_BROWSER_ALLOW_EVALUATE/);
...
test("embeds a live Chromium view inside the existing desktop browser panel", () => {
  ...
  assert.match(desktopBrowser, /ALLOWED_EXTERNAL_PROTOCOLS/);
  assert.match(desktopBrowser, /Blocked unsupported browser protocol/);
  assert.match(desktopBrowser, /will-redirect/);
  assert.match(desktopBrowser, /PIORA_BROWSER_ALLOW_EVALUATE/);
  assert.doesNotMatch(desktopBrowser, /else void shell\.openExternal\(url\)/);
