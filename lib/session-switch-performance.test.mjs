...
test("coalesces and reuses unchanged session response projections", () => {
  ...
  assert.match(routeSource, /signature = `\$\{fileState\.size\}:\$\{fileState\.mtimeMs\}:\$\{readPendingSessionModel\(id\)\?\.revision \?\? ""\}:\$\{getRpcSession\(id\)\?\.isRunning\(\) \?\? false\}:\$\{commandState\?\.size \?\? 0\}:\$\{commandState\?\.mtimeMs \?\? 0\}`/);
