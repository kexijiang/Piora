...
test("Pocket prepares clipboard and transfer data on home and preserves them across tool switches", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    const activateTab = async (name) => {
      ...
      await page.waitForFunction((label) => [...document.querySelectorAll('[role="tab"]')].some((button) => button.textContent?.trim() === label && button.getAttribute("aria-selected") === "true"), name);
      assert.equal(await target.getAttribute("aria-selected"), "true");
    };
    await activateTab("剪贴板");
    assert.ok(await page.getByRole("listbox", { name: "剪贴板记录" }).getByRole("option").count() > 0);
    await activateTab("中转站");
