...
test("independent companion bubble follows session snapshots inside the native window bounds", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await page.route("http://companion-bubble.test/**", (route) => {
      ...
      if (url.pathname === "/api/agent/running") return route.fulfill({ json: { runningSessions } });
    ...
    await page.exposeFunction("setTasks", tasks => { runningSessions = tasks; });
    await page.addInitScript(() => {
      window.publishTasks = async tasks => { await window.setTasks(tasks); window.dispatchEvent(new Event("online")); };
    });
