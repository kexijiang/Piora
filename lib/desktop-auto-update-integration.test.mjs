...
test("installed Piora preserves manual checks and supports scheduled silent installation", () => {
  ...
  assert.match(main, /await shutdownPromise[\s\S]*?quitAndInstall\(silent\)/);
  assert.match(main, /new ScheduledDesktopUpdater/);
  assert.match(main, /updateBlockers\.size > 0/);
