...
test("system clipboard capture defaults on while an explicit pause survives reopening", async t => {
  const { root, db } = await fixture(t);
  assert.equal(db.settings().enabled, true);
  db.mutate({ type: "settings", value: { enabled: false } });
  const reopened = new ClipboardDatabase(root);
  try { assert.equal(reopened.settings().enabled, false); } finally { reopened.close(); }
});
