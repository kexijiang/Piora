...
test("browser backup preserves custom background blobs and original drafts across restore/reload; quota failures roll back", async () => {
  ...
  try {
    ...
    // Data URL imports are evaluated asynchronously in Chromium.  Wait for
    // the module's public bridge instead of racing the first page evaluation.
    await page.waitForFunction(() => typeof window.backupClient?.setDraft === "function" && typeof window.backupClient?.snapshotClientBackup === "function");
    ...
    await page.waitForFunction(() => typeof window.backupClient?.getDraft === "function" && typeof window.backupClient?.snapshotClientBackup === "function");
