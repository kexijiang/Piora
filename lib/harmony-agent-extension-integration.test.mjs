...
function loadTools() {
  ...
  assert.equal(tools.size, 1, "Harmony exposes only one model-facing entry");
  const gateway = tools.get("harmony_control");
  return { get(name) {
    if (name === "harmony_control") return gateway;
    return { execute(id, input, signal, update, ctx) {
      return gateway.execute(id, { operation: name.replace(/^harmony_/, ""), input }, signal, update, ctx);
    } };
  } };
}
...
test("the complete Harmony entry stays below 600 estimated tokens", () => {
  const scenario = loadTools().get("harmony_control");
  assert.ok(scenario);
  assert.ok(estimateToolDefinitionPromptTokens([scenario]) < Math.min(600, TOOL_DEFINITION_PROMPT_TOKEN_LIMIT));
});

test("Harmony operation schemas load on demand and invalid inputs fail before device access", async () => {
  const gateway = loadTools().get("harmony_control");
  const help = await gateway.execute("help", { operation: "help", topic: "run_scenario" });
  const schema = JSON.parse(help.content[0].text);
  assert.ok(schema.inputSchema.properties.steps);
  await assert.rejects(() => gateway.execute("invalid", { operation: "input_text", input: { text: "" } }), /text|validation/i);
  await assert.rejects(() => gateway.execute("invalid", { operation: "missing" }), /Unknown/);
});
...
test("real tool execution keeps leases opaque and releases them at final run idle", async (t) => {
  ...
  const swipeStart = calls.length;
  await tools.get("harmony_swipe").execute("call-swipe", {
    serial: "phone-1", direction: "up",
  }, signal, undefined, ctx);
  assert.ok(calls.slice(swipeStart).some((call) => call[0] === "swipe"));
  const swipeSnapshots = calls.slice(swipeStart).filter((call) => call[0] === "snapshot");
  assert.ok(swipeSnapshots.length > 0);
  assert.ok(swipeSnapshots.every((call) => call[1].includeTree && !call[1].includeScreenshot));
