...
function harness(onSend) {
  ...
  const env = { useCallback: (fn) => fn, useEffect() {}, submitRef: { current: null }, sendingRef: { current: false },
    ...
    localVoiceActiveRef: { current: false }, localVoiceStopRef: { current: async () => true }, speechInsertionRef: { current: null },
    ...
    drafts: new Map(),
    setDraft: (key, draft) => env.drafts.set(key, draft),
    getDraft: key => env.drafts.get(key) ?? null,
    deferDraftPersistence: () => () => {},
...
test("actual composer clears before local receipt and late receipts preserve newer typing", async () => {
  ...
  assert.equal(env.valueRef.current, "");
  env.valueRef.current = "new draft";
  acknowledge(); assert.equal(env.valueRef.current, "new draft");
...
test("queued submissions clear immediately and restore failures without overwriting edited or switched drafts", async () => {
  ...
  for (const mode of ["steer", "followup", "slash"]) {
    for (const outcome of ["accepted", "rejected", "thrown", "edited", "switched"]) {
      ...
      const env = {
        ...
        replyDraftRef: { current: { value, spans: [] } }, retryOfPromptIdsRef: { current: [] },
        resetReplyDraft: draft => { env.valueRef.current = draft.value; },
        drafts: new Map(), setDraft: (key, draft) => env.drafts.set(key, draft),
        getDraft: key => env.drafts.get(key) ?? null,
        deferDraftPersistence: () => () => {},
      ...
      assert.equal(env.valueRef.current, "", "guidance clears before the queue reply");
      if (outcome === "edited") env.valueRef.current = "newer draft";
      if (outcome === "switched") env.draftKeyRef.current = "other-session";
      finish(); await pending;
      assert.equal(env.valueRef.current, outcome === "accepted" || outcome === "switched" ? "" : outcome === "edited" ? "newer draft" : value, `${mode}: ${outcome}`);
