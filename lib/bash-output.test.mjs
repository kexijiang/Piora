...
test("accepts only Pi shell logs directly inside the configured temp directory", async () => {
  ...
  assert.equal(resolveBashOutputPath(join(tempRoot, "pi-powershell-ab12.log"), tempRoot), resolve(tempRoot, "pi-powershell-ab12.log"));
  ...
  assert.equal(resolveBashOutputPath(join(tempRoot, "pi-shell-ab12.log"), tempRoot), null);
