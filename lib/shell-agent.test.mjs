...
test("approval binds the exact command and terminal, and stale answers cannot execute it", { timeout: 60000 }, async t => {
  ...
  assert.equal(waiting.approval.command, command); assert.equal(waiting.approval.cwd, directory); assert.equal(waiting.approval.generation, session.state.generation);
