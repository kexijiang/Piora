...
test("settings control browser mode, update the open panel and retain state after failed saves", { timeout: 120000 }, async () => {
  ...
  try {
    ...
    await writeFile(path.join(root, "entry.tsx"), `import {createRoot} from "react-dom/client";import {I18nProvider} from ${JSON.stringify(path.join(repo, "hooks/useI18n.tsx"))};import {BrowserPanel} from ${JSON.stringify(path.join(repo, "components/workspace/BrowserPanel.tsx"))};import {BrowserModeSetting} from ${JSON.stringify(path.join(repo, "components/BrowserModeSetting.tsx"))};createRoot(document.getElementById("root")).render(<I18nProvider><div id="settings"><BrowserModeSetting/></div><div id="browser-panel" style={{height:500}}><BrowserPanel active={true} maximized={false} sessionId="test-session"/></div></I18nProvider>);`);
    ...
    let mode = "builtin", connects = 0, rejectSave = true;
    const actions = [];
    ...
    await page.route("http://browser-mode.test/**", async (route) => {
      ...
      if (url.pathname === "/api/browser/settings") {
        if (route.request().method() === "PUT") {
          if (rejectSave) return route.fulfill({ status: 500, json: { error: "Could not save browser mode" } });
    ...
    const toggle = page.locator('#settings [data-settings-id="general.browser"]').getByRole("switch", { name: "使用后台浏览器" });
    assert.equal(await page.locator("#browser-panel").getByRole("switch").count(), 0, "mode control belongs only to Settings");
    assert.equal(await toggle.isChecked(), false);
    await toggle.click();
    await page.getByRole("alert").waitFor();
    assert.equal(await toggle.isChecked(), false, "a failed save must retain the previous mode");
    assert.equal(await page.locator('[data-native-browser="true"]').count(), 1);
    rejectSave = false;
