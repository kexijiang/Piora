...
test("Electron startup cinematic stays local, escapable, muted and motion-aware", () => {
  ...
  assert.doesNotMatch(document, /piora-startup:\/\/continue|location\.href/);
  assert.match(document, /finishStartupIntro/);
  assert.match(document, /&lt;bad&gt;&amp;&quot;/);
  assert.match(document, /更新就绪/);
  assert.match(desktopMain, /await startup\.finished;\s*recordStartupStage\("loading-main-window"\);\s*await loadApplicationWindow/);
...
test("intro playback completion, media failure and reduced motion all release startup", () => {
  ...
  function playback(reducedMotion = false) {
    ...
    let signals = 0;
    const button = { addEventListener: (name, handler) => handlers.set(name, handler) };
    runInNewContext(script, { document: { querySelector: selector => selector === "video" ? clip : button }, matchMedia: () => motion, window: { piDesktop: { finishStartupIntro() { signals++; } } } });
    return { handlers, changes, motion, signals: () => signals, pauses: () => pauses };
  }
  for (const event of ['ended', 'error', 'click']) {
    const player = playback();
    assert.equal(player.signals(), 0);
    player.handlers.get(event)();
    assert.equal(player.signals(), 1);
    player.handlers.get("error")(); player.handlers.get("ended")();
    assert.equal(player.signals(), 1, "completion is one-shot even when media emits again");
  }
  const reduced = playback(true);
  assert.equal(reduced.pauses(), 1);
  assert.equal(reduced.signals(), 1);
  const changed = playback();
  changed.motion.matches = true;
  changed.changes.get('change')();
  assert.equal(changed.pauses(), 1);
  assert.equal(changed.signals(), 1);
});
