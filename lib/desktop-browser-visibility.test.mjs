...
function setup(t) {
  ...
  const win = Object.assign(new EventEmitter(), {
    ...
    webContents: { isDestroyed: () => false, getZoomFactor: () => 1.25, send() {} },
    contentView: {
      children: [],
      addChildView(view) {
  ...
  const browserSession = Object.assign(new EventEmitter(), {
    ...
    flushStorageData() {},
    setDownloadPath() {},
    allowNTLMCredentialsForDomains(domains) { this.authDomains = domains; },
    setPermissionCheckHandler() {},
    setPermissionRequestHandler() {},
  });
  const electron = {
    Menu: { buildFromTemplate(template) {
      const menu = { template, popup(options) { this.options = options; } };
      menus.push(menu);
      return menu;
    } },
  ...
  return {
    manager, win, views, logs, handlers, api, menus, browserSession,
    ...
    bookmarks: (nodes, point = { x: 500, y: 90 }) => invoke(api.BROWSER_BOOKMARK_MENU_CHANNEL, nodes, point),
  };
}

test("integrated login is scoped to the challenged HTTPS host and starts disabled", async (t) => {
  const { action, views, menus, browserSession } = setup(t);
  assert.equal(browserSession.authDomains, "");
  await action({ action: "navigate", url: "https://portal.corp.test/" });
  views[0].webContents.emit("login", {}, { url: "https://sso.corp.test/auth" }, { scheme: "Negotiate", isProxy: false, host: "sso.corp.test" });
  await action({ action: "configure_login" });
  assert.equal(menus.at(-1).template[0].label, "sso.corp.test");
  assert.equal(menus.at(-1).template[1].checked, false);
  assert.equal(browserSession.authDomains, "", "opening the menu must not send system credentials");
});

test("bookmark folders use anchored native submenus without changing the live viewport", async (t) => {
  const { action, viewport, views, win, bookmarks, menus } = setup(t);
  await action({ action: "navigate", url: "https://example.test/" });
  viewport(true);
  const originalBounds = views[0].bounds;
  const selection = bookmarks([{ type: "folder", title: "Work & tools", children: [
    { type: "bookmark", title: "Docs", url: "https://example.test/docs" },
  ] }]);
  const menu = menus[0];
  assert.equal(menu.template[0].label, "Work && tools");
  assert.equal(menu.options.x, 625);
  assert.equal(menu.options.y, 113);
  assert.equal(views[0].bounds, originalBounds);
  assert.deepEqual(win.contentView.children, [views[0]]);
  menu.template[0].submenu[0].click();
  menu.options.callback();
  assert.equal(await selection, "https://example.test/docs");
  const dismissed = bookmarks([]);
  assert.equal(menus[1].template[0].enabled, false);
  menus[1].options.callback();
  assert.equal(await dismissed, null);
});

test("bookmark IPC rejects untrusted callers, unsafe URLs and invalid anchors", (t) => {
  const { bookmarks, handlers, api, menus } = setup(t);
  assert.equal(handlers.get(api.BROWSER_BOOKMARK_MENU_CHANNEL)({}, [], { x: 0, y: 0 }), null);
  assert.throws(() => bookmarks([{ type: "bookmark", title: "bad", url: "javascript:alert(1)" }]), /Invalid bookmark URL/);
  assert.throws(() => bookmarks([], { x: NaN, y: 0 }), /Invalid bookmark menu position/);
...
test("navigation and managed login popups cannot reopen a collapsed panel", async (t) => {
  ...
  await views[1].webContents.loadURL("https://account.example.test/popup");
