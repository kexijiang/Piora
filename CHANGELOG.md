...
## [0.5.2-beta.9] - 2026-09-23

### 任务执行状态

- 切换会话后返回正在执行的 bash 命令时，读秒从实际启动时间继续；等待模型、生成回复、运行工具、扩展命令和停止中的阶段读秒及无进展提示也恢复原有时间，不再从切回时重新计算。运行中追加指令不再重置本轮总计时。

### 设置中心

- 恢复左侧“搜索设置”输入框的正常高度与宽度，避免被长导航列表挤小。
- 修复宠物设置嵌入页面时误用弹窗遮罩导致整页发灰，并统一标题、留白和设置卡片样式。
- 统一定时任务、插件和技能页的标题、内容间距及列表表单样式；移除重复的“项目能力”汇总页，具体功能仍由一级菜单直达。

### 文件工作区

- `.ets` ArkTS、`.json5` 配置和 `.hml` 页面文件在编辑器、源码预览与差异视图中显示语法高亮。
- 右侧文件面板首次打开后保留挂载状态；关闭再打开时沿用文件树展开、筛选和滚动位置，不再重新加载整棵目录。
- 文件内 Ctrl+F 改为编辑器右上角的紧凑查找框，支持匹配计数、查找选项和按需展开替换。
- ArkTS、TypeScript 和 JavaScript 文件支持 Ctrl+点击/F12 跳转定义与代码补全；目标在文件标签中打开并定位。ArkTS 语义服务不可用时显示基础模式并提供有限的符号跳转与提示。

## [0.5.2-beta.8] - 2026-09-23
...
- 修复 JSON 编辑器在输入法组字时反复同步中间文本、重新配置编辑器可能造成重复字符的问题；保留大括号、方括号和引号的自动配对，组字结束后再同步最终内容。

### 文件工作区

- 文件页改为可拖动的左右分栏，文件树宽度可记忆；窄面板自动收为可展开栏。右侧工作区默认更宽，也可继续向左拉，同时保留对话空间。
- 多文件标签支持自动显露和全部文件菜单，切换时保留未保存草稿；Markdown 和 HTML 可在源码旁预览当前草稿，窄窗口可在编辑与预览间切换。
- 新增“本轮结果”文件列表，按一次任务开始与结束时的文件状态展示期间发生的净变化，保留最近一轮结果供刷新后查看，并可跳转完整差异。
- 文件标签溢出时改用轻量的左右滚动按钮、边缘渐隐提示和滚轮横向滚动；文件树按新增、修改、删除分别着色，文件夹汇总显示下方文件的多种变更状态。
- 文件编辑区加入按扩展名识别的代码语法高亮、折叠、括号匹配与查找，并在行号旁用色条提示 Git 变更和当前未保存草稿中的改动。
- 修正 CRLF 文件编辑一行后整文件被误标为修改的问题；编辑器保留原有换行方式，并将代码折叠按钮改为居中的轻量箭头。
- 修复桌面开发版连续热更新后文件区样式丢失、文件树与编辑器重叠的问题；文件区样式随页面稳定加载，并为关键分栏布局提供兜底。
- 修复新增行紧挨修改行时两行都被标成修改的问题；根据原行与新行的内容配对，相邻新增行保持绿色标记。

### 设置中心

- 模型配置的运行策略新增上下文自动压缩阈值；切换到上下文窗口较小的模型时，若当前会话超出目标窗口，会先压缩并确认可容纳后再切换，压缩失败则保留原模型。
- 将设置导航整理为使用体验、模型与工作流、能力扩展、连接与设备、应用与数据五组；语言并入外观与界面，取消独立“通用”页面。
- 设置左侧恢复为始终展开的完整页面列表；模型运行策略、项目工具、插件、归档等入口直接显示，窄屏也可在横向导航中直接选择。
- 将模型重试与回退、提示词与标题、网络代理、数据迁移与诊断等选项归入对应页面；项目工具、能力包、扩展、插件、归档与回收站分别展示入口，并更新设置搜索与窄屏导航。

### 任务执行稳定性

- 新增 Shell Guard：模型执行 bash/PowerShell 命令时自动补上默认 10 分钟超时，并保留已配置的 Bash 路径与命令前缀。对可识别的前台 dev server、watch、`tail -f` 及交互式 Git 命令提示改用后台或显式超时；按实际命令和参数判断，避免误拦截版本查询、帮助和包含命令字样的搜索文本。
- 新增模型请求停滞看门狗：首响应等待和流式输出静默遵循设置中的超时，支持延长或关闭限制，并同步底层 HTTP 连接配置；停滞时以“请求超时”交给自动重试与模型回退处理。用户点击停止时立即结束等待，即使服务端完全没有输出，也不会被误判为超时而触发重试。
- 运行状态行新增已运行时长、自动重试进度（第 n/m 次及原因）和“已 X 秒没有输出或新进展”提示；bash 挂起和模型停滞都能在界面上直接看出来。
- 设置 > 模型配置 > 运行策略新增“模型请求超时与重试”：可视化配置自动重试次数与退避、请求首响应超时、HTTP 空闲超时和服务层重试，写入全局 `settings.json`。保存后刷新已有会话的设置，后续请求和重试使用新值，无需重建会话；正在执行的请求保留原超时，项目覆盖设置继续优先。
- 修复刷新页面后仍在执行的 bash 命令被误标为“已中断”的问题：只有服务端命令注册表确认未在运行才降级状态，静默运行中的命令保持运行标记。

### 对话体验

- 点击发送或按发送快捷键后立即清空输入框，不再等待本地恢复副本写入或模型切换完成；保留原稿到恢复副本保存完成，发送失败时恢复文字和附件，且不会覆盖新输入或其他会话的草稿。
- 运行中追加引导或排队消息同样立即清空输入框，服务端接收前保留原稿，提交失败时安全恢复。

### 发布与测试

- 修复模型停滞看门狗在独立测试进程中使用非保活计时器，进程可能在等待超时期间提前退出的问题；超时计时器现在会持续到响应、取消或超时结束，相关 14 项测试全部通过。
- 定时任务、项目工具、用量统计和外观入口测试改为检查实际使用的设置导航注册表；同步 Shell Guard 扩展清单与新增压缩设置后的测试边界，并移除已取消的设置工具栏样式与断言，避免界面重构后因旧测试误报。
- 会话运行测试改为等待本轮文件扫描实际完成，再断言结束、取消与远程状态；文件记录增加异步扫描后，测试不再依赖单次事件循环恰好完成。
- 同步根与桌面版本、锁文件、README 和许可证清单，并修正 README 中运行日志的设置路径，使安装包版本及更新说明对应同一个 beta 发布提交。

## [0.5.2-beta.7] - 2026-09-23

此标签的 Windows 测试分片在验证模型停滞看门狗时，有 13 项等待超时的测试因计时器未保持进程运行而被取消，发布门禁失败；未创建 GitHub Release。完整更新内容与修复归入 0.5.2-beta.8。
